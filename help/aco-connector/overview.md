...
설정 및 구성을 위한 높은 수준의 단계는 다음과 같습니다.

1. Adobe Commerce용 커넥터 패키지를 설치합니다.

1. 인증 및 환경 세부 정보를 구성합니다.

1. Commerce 범위를 Commerce Optimizer에 매핑합니다.

1. 연결을 확인합니다.

자세한 지침은 _시작_ 안내서에서 [구성 단계](./get-started.md#configuration-steps)를 참조하십시오.
...
Adobe Commerce Optimizer 커넥터에 다음 피드를 사용할 수 있습니다.
...
- `prices` - 제품 가격
- `categories` - 범주 데이터
다시 동기화 작업에 Commerce 명령줄 인터페이스(CLI)를 사용하는 방법에 대한 자세한 내용은 [CLI 다시 동기화 명령](../data-export/data-export-cli-commands.md#sync-using-cli-commands){target="blank"}을 참조하십시오.
