...
다음 항목은 2026년 4월 1일에 프로덕션 환경에 릴리스됩니다.

>[!BEGINSHADEBOX]

### GraphQL을 통해 가격 및 스톡 경고 구독 상태 확인

새 GraphQL 쿼리 `isSubscribedProductAlertStock` 및 `isSubscribedProductAlertPrice`을(를) 통해 상점 첫 화면에서 쇼핑객이 이미 제품에 대한 주식 또는 가격 알림을 구독했는지 여부를 확인할 수 있습니다. <!-- ACCS-334 -->
...
`recaptchaFormConfigs` 쿼리는 양식당 하나의 쿼리가 필요하지 않고 단일 요청으로 여러 양식 유형에 대한 구성을 반환할 수 있습니다. <!-- ACCS-628 -->
