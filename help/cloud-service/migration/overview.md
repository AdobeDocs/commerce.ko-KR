...
[!DNL Adobe Commerce as a Cloud Service]은(는) 기존 Adobe Commerce PaaS 구현에서 새로운 Adobe Commerce as a Cloud Service(SaaS) 서비스로 전환하는 개발자를 위한 포괄적인 안내서를 제공합니다. Adobe Commerce as a Cloud Service은 향상된 성능, 확장성, 간소화된 작업, 더 광범위한 [!DNL Adobe Experience Cloud]과의 긴밀한 통합을 제공하는 완벽한 관리 버전 없는 SaaS 모델로의 대폭적인 전환을 나타냅니다.
...
* [!BADGE SaaS만 해당]{type=Positive url="https://experienceleague.adobe.com/ko/docs/commerce/user-guides/product-solutions" tooltip="Adobe Commerce as a Cloud Service 및 Adobe Commerce Optimizer 프로젝트에만 적용됩니다(Adobe 관리 SaaS 인프라)."} **SaaS(신규 - [!DNL Adobe Commerce as a Cloud Service])**: Adobe에서 핵심 응용 프로그램, 인프라 및 업데이트를 완전히 관리합니다. 판매자는 확장성 지점(API, 앱 빌더, UI SDK)을 통한 사용자 지정에 대한 포커스. 핵심 애플리케이션 코드가 잠겨 있습니다.
...
* [Adobe Systems Developer 앱 Builder용 Adobe Systems Developer 앱 Builder](https://developer.adobe.com/app-builder/) 및 [API Mesh](https://developer.adobe.com/graphql-mesh-gateway)
* [Commerce Optimizer](../../optimizer/overview.md)
* [엣지 딜리버리 서비스](https://experienceleague.adobe.com/developer/commerce/storefront/?lang=ko)
* Commerce Cloud Manager를 [통한 셀프 서비스 프로비저닝](../getting-started.md#create-an-instance)

## 마이그레이션 경로

[!DNL Adobe Commerce as a Cloud Service] 은 타임라인, 상점 및 사용자 지정에 따라 여러 마이그레이션 경로를 지원합니다.

전체 마이그레이션 [!DNL Adobe Commerce as a Cloud Service] 대신 Commerce Optimizer 또는 증분 접근 방식을 사용하는 단계적 마이그레이션을 지원합니다.

* **증분 마이그레이션** - 이 접근 방식에는 데이터, 사용자 지정 및 통합을 단계적으로 마이그레이션하는 작업이 포함됩니다. 이 접근 방식은 복잡한 사용자 지정 및 데이터를 [!DNL Adobe Commerce as a Cloud Service] 자신의 속도로 점진적으로 전환하려는 많은 사용자 정의가 있는 대규모 판매자에게 이상적입니다.

![증분 마이그레이션](../assets/incremental.png){width="600" zoomable="yes"}

* **Commerce Optimizer** - 이 접근 방식을 사용하면 Commerce Optimizer를 전환 단계로 사용하여 복잡한 사용자 지정 및 데이터를 원하는 속도로 이동하여 반복적으로 마이그레이션할 [!DNL Adobe Commerce as a Cloud Service] 수 있습니다. Commerce Optimizer는 카탈로그 보기 및 정책에서 제공하는 머천다이징 서비스, 에지 배달에서 제공하는 상거래 상점 및 [!DNL Product Visuals powered by AEM Assets]에 대한 액세스를 제공합니다.
...
| 점포 | Edge Delivery를 기반으로 하는 Commerce Storefront로 마이그레이션 | 에지 딜리버리 기반의 Commerce Storefront로 마이그레이션 또는 유지 관리 | 영향 없음 | 영향 없음 |
| API 메시 | 새 메시 작성 | 새 메쉬 구축 또는 기존 메쉬 재구성 | 새 메쉬 구축 또는 기존 메쉬 재구성 | 새 메쉬 구축 또는 기존 메쉬 재구성 |
| 통합 | 통합 스타터 키트 활용 | 통합 스타터 키트 활용 | 통합 스타터 키트 활용 | 통합 스타터 키트 활용 |
...
표에 표시된 대로 각 마이그레이션에 대한 완화는 다음과 같이 구성됩니다.

* **데이터 마이그레이션** - 제공된 [마이그레이션 도구를](./bulk-data.md) 사용하여 기존 인스턴스 [!DNL Adobe Commerce as a Cloud Service]데이터를 .
...
* **Assets 관리** - 모든 자산 관리에는 마이그레이션이 필요합니다. 이미 사용 [!DNL AEM Assets]중인 경우에는 마이그레이션할 필요가 없습니다.
* **확장** - 모든 in-process 확장은 out-of-process 확장으로 다시 만들어야 합니다. 2025년 말까지 Adobe Systems 는 빌드 시간을 최소화하기 위해 가장 인기 있는 확장에 대한 액세스 권한을 제공할 예정입니다.

## 마이그레이션 단계

다음 단계에서는 로 마이그레이션 [!DNL Adobe Commerce as a Cloud Service]하는 데 필요한 단계와 고려 사항에 대해 설명합니다.
...
* 핵심 코드 수정 사항을 분석하고 마이그레이션의 일부로 리팩터링이 필요한 사항을 결정합니다.
...
* 현재 Lighthouse 점수, 페이지 로드 시간 및 KPI(핵심 성과 지표)를 문서화하여 게시물 마이그레이션 후 개선 사항을 측정하기 위한 기준을 제공합니다.
...
* **핵심 비즈니스 프로세스 식별:** 다음과 같이 먼저 마이그레이션해야 하는 기능의 우선 순위를 지정합니다.
   ...
   * 복잡한 세금 계산
   * 주소 유효성 검사
   * 주문 후 트리거되는 사용자 지정 논리
* **헤드리스 vs. 모놀리식 매장:** 새로운 매장을 개발하거나 기존 매장을 조정하기 위한 의사 결정 지점입니다.
* **통합 전략:** 기존 통합의 플랫폼을 재구성하는 방법을 결정합니다(API 메시, 앱 빌더, 직접 API).
* **데이터 마이그레이션 전략:** 전체 내역 데이터, 부분 데이터 또는 마이그레이션된 데이터를 사용하지 않고 마이그레이션할지 결정합니다.

**팀 준비 및 교육:**

* 개념, 개발 워크플로우 및 새로운 도구에 익숙 [!DNL Adobe Commerce as a Cloud Service] 해지십시오.
...
**이점**: 이 단계는 심층적으로 포함된 사용자 지정과 관련된 기술 부채를 크게 줄이고, Commerce 인스턴스 를 [!DNL Adobe Commerce as a Cloud Service]로 전환하는 속도를 크게 높이고, 사용자 지정 로직의 확장성 및 독립적인 배포 가능성을 향상시키고, 확장에 대한 개발 주기를 단축합니다.

#### &#x200B;2. SaaS 기반 Adobe Systems Commerce 머천다이징 서비스 채택 및 카탈로그 데이터 통합

이는 카탈로그 데이터 관리와 관련된 두 가지 옵션이 있는 중요 초기 통합 지점입니다.
...
이는 Adobe Systems Commerce Optimizer를 활용하기 위한 전략적이고 미래 지향적인 접근 방식입니다. CCDM은 다중 소스 데이터 수집 및 동적 머천다이징을 위해 설계된 유연하고 확장 가능한 통합 카탈로그 서비스를 제공합니다.

* **데이터 수집 및 통합**
   * 기존 Adobe Systems Commerce PaaS 인스턴스(및/또는 기타 PIM/ERP 시스템)의 제품 및 카탈로그 데이터를 새로운 CCDM(Composable Catalog Data Model)으로 수집하는 것으로 시작합니다.
   * 기존 제품 속성을 CCDM의 유연한 스키마에 매핑합니다. 초기 수집을 위해 중요 제품 데이터의 우선 순위를 지정합니다.
...
* **초기 설정**: Edge Delivery Services용 Adobe Systems Commerce Storefront 상용구를 사용하여 프로젝트를 설정합니다. 이는 최신 웹 기술을 기반으로 구축된 기본적인 헤드리스 프론트엔드를 제공합니다.
* **카탈로그 서비스 및 API Mesh**&#x200B;에 연결: Commerce Storefront는 주로 GraphQL API를 통해 데이터를 사용합니다.
...
* **컨텐츠 마이그레이션(AEM 서비스):** 기존의 정적 컨텐츠(예: &quot;회사 정보&quot; 페이지, 블로그 게시물 및 마케팅 배너)를 상거래 상점을 지원하는 AEM 서비스로 마이그레이션합니다. AEM의 콘텐츠 작성 기능을 활용하고 자산이 엣지 딜리버리 서비스에 최적화되도록 합니다.
...
* **기존 장바구니/체크아웃과 통합**: 처음에는 Edge Delivery Services 상점 측에서 장바구니 관리 및 체크아웃을 위해 기존 Adobe Commerce PaaS(또는 다른 타사 플랫폼)에 대한 핸드오프를 조정합니다. 이 작업에는 일반적으로 다음이 포함됩니다.
   ...
   * **직접 API 상호 작용** (앱 Builder 오케스트레이션 사용): PaaS 백엔드의 장바구니 및 체크아웃 API와 직접 상호 작용하는 Edge Delivery Services 내에서 사용자 지정 장바구니 및 체크아웃 UI 구성 요소를 빌드합니다. 여기에는 여러 백엔드 서비스(예: PaaS 장바구니, 결제 게이트웨이, 배송 계산기)에 대한 호출을 오케스트레이션하기 위해 앱 빌더를 BFF(백엔드용 프론트엔드)로 사용하는 경우가 많습니다.

**이점**: 매우 빠르고, SEO 최적화되고, 매우 유연한 매장 경험 제공. 이 단계는 우수한 고객 경험에 직접적으로 기여하고 미래의 프론트엔드 혁신을 위한 토대를 마련합니다.

#### &#x200B;4. 데이터 마이그레이션(단계별 프로세스)

데이터 마이그레이션은 리팩터링 및 상점 개발과 동시에 실행되는 중요하고 다면적인 프로세스로, 데이터 일관성과 무결성을 보장합니다.
...
* **도구 가용성**: 자사 Commerce 대량 데이터 마이그레이션에 고객이 사용할 전용 [대량 데이터 마이그레이션 도구](./bulk-data.md)는 2026년 1분기에 요청에 의해 사용할 수 있습니다. 고객이 미리 대량 데이터 마이그레이션에 대한 지원이 필요한 경우 Adobe에서 요청에 의해 고객을 대신하여 데이터를 쉽게 전송할 수 있습니다.
...
* **도구 가용성**: 반복적인 데이터 마이그레이션을 위해 특별히 고안된 도구는 2026년에 사용할 수 있습니다.
...
* 유효성 검사 기간 후에 이전 PaaS 인스턴스 및 데이터를 안전하게 보관하거나 삭제합니다.

**진행 중인 개발 작업 과정:**

* 대규모 업그레이드가 아닌 지속적인 소규모 배포가 있는 의 버전 없는 특성을 [!DNL Adobe Commerce as a Cloud Service]수용합니다.
* Cloud Manager를 활용하여 환경 및 배포를 관리할 수 있습니다.
* 앱 빌더를 활용하여 코어에 영향을 주지 않고 기능을 확장할 수 있습니다.
