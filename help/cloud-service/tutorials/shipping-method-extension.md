...
이 튜토리얼에서는 [!DNL Adobe App Builder], [체크아웃 스타터 키트](https://developer.adobe.com/commerce/extensibility/starter-kit/checkout/){target="_blank"} 및 AI 지원 개발 도구를 사용하여 [!DNL Adobe Commerce as a Cloud Service]에 대한 배송 방법 확장을 빌드하는 방법을 안내합니다.
...
1. Pipedream은 `https://123456.m.pipedream.net`과(와) 같은 고유한 **HTTP 끝점 URL**&#x200B;을(를) 표시합니다.
...
이 섹션에서는 [체크아웃 스타터 키트](https://developer.adobe.com/commerce/extensibility/starter-kit/checkout/){target="_blank"} 및 AI 지원 개발 도구를 사용하여 [!DNL Adobe Commerce as a Cloud Service]용 배송 방법 확장을 개발하는 방법을 안내합니다.
...
1. `requirements.md`(또는 이에 상응하는) 파일을 검토하고 계획을 확인합니다. 모든 것이 올바르게 보이는 경우 에이전트에게 아키텍처 계획(또는 **단계 2**)으로 이동하도록 지시합니다. 다음을 확인합니다.
   ...
   - **shipping-config**(또는 이와 동등한 작업)은 구성이 안전하게 저장된 GET(읽기 구성, API 키 마스킹된) 및 SET(저장 서비스 URL, API 키, 웨어하우스 주소)를 지원합니다(예: 런타임 상태).
...
1. 코드 생성을 진행하도록 에이전트에 지시합니다. 에이전트는 Commerce에서 반환된 메서드를 수락하고 웹후크 메서드 `plugin.magento.out_of_process_shipping_methods.api.shipping_rate_repository.get_rates`(웹후크 유형 **후**, 필수 **선택 사항**)를 사용할 수 있도록 하는 **mock** 캐리어를 배송 회사 구성에 추가해야 합니다.
...
1. 생성된 코드를 확인한 후 다음 프롬프트를 사용하여 확장을 배포합니다.
   ...
   에이전트는 배포 전 준비 상태 평가를 수행합니다(예: 관리 UI 또는 Commerce API가 사용되는 경우 `COMMERCE_WEBHOOKS_PUBLIC_KEY`, `COMMERCE_BASE_URL` 및 OAuth/IMS 변수에 대한 `.env` 확인).
