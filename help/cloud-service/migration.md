---
title: Adobe Commerce as a Cloud Service으로 마이그레이션
description: Adobe Commerce as a Cloud Service으로 마이그레이션하는 방법에 대해 알아봅니다.
...
Adobe Commerce as a Cloud Service은 대부분의 구성을 즉시 제공합니다. 그러나 기존 Adobe Commerce on Cloud 또는 온프레미스 인스턴스에서 마이그레이션하는 경우 특정 구성에 따라 다른 마이그레이션 작업을 수행해야 합니다.

## 마이그레이션 경로

Adobe Commerce as a Cloud Service은 타임라인, 상점 및 사용자 지정에 따라 여러 마이그레이션 경로를 지원합니다.

전체 마이그레이션에 대한 대안으로 Adobe Commerce as a Cloud Service은 Commerce Optimizer 또는 증분 접근 방식을 사용하여 단계별 마이그레이션을 지원합니다.

* **증분 마이그레이션** - 이 방법에는 데이터, 사용자 지정 및 통합을 단계적으로 마이그레이션하는 작업이 포함됩니다. 이 접근 방식은 복잡한 사용자 정의 및 데이터를 원하는 속도로 Adobe Commerce as a Cloud Service으로 점진적으로 전환하려는 많은 사용자 정의 기능을 가진 대형 가맹점에 이상적입니다.

![증분 마이그레이션](./assets/incremental.png){width="600" zoomable="yes"}

* **Commerce Optimizer**—이 접근 방식을 사용하면 Commerce Optimizer을 전환 단계로 사용하여 복잡한 사용자 지정 및 데이터를 원하는 속도로 Adobe Commerce as a Cloud Service으로 이동하여 반복적으로 마이그레이션할 수 있습니다. Commerce Optimizer은 카탈로그 채널 및 정책으로 구동되는 머천다이징 서비스, Edge Delivery으로 구동되는 Commerce Storefront 및 AEM Assets으로 구동되는 제품 비주얼에 대한 액세스를 제공합니다.

![반복 마이그레이션](./assets/optimizer.png){width="600" zoomable="yes"}

* **전체 마이그레이션**—이 방법에는 모든 데이터, 사용자 지정 및 통합을 한 번에 마이그레이션하는 작업이 포함됩니다. 이 방법은 Adobe Commerce as a Cloud Service으로 빠르게 전환하려는 사용자 정의가 거의 없는 소규모 가맹점에 이상적입니다.
...
* **데이터 마이그레이션** - 제공된 마이그레이션 도구를 사용하여 기존 인스턴스에서 Adobe Commerce as a Cloud Service으로 데이터를 마이그레이션합니다.
...
* **통합**—모든 통합은 [통합 시작 키트](https://developer.adobe.com/commerce/extensibility/starter-kit/integration/) 또는 [Adobe Commerce as a Cloud Service REST API](https://developer.adobe.com/commerce/services/reference/cloud-service/core-admin/)를 활용해야 합니다.
...
* **확장**—모든 In-Process 확장을 Out-of-Process 확장으로 다시 만들어야 합니다. 2025년 말까지 Adobe은 미리 로드되고 빌드 시간을 최소화하기 위해 사용할 준비가 된 100개의 가장 인기 있는 확장을 제공합니다.

## 마이그레이션 단계

현재 Adobe Commerce 인스턴스에서 새 Adobe Commerce as a Cloud Service 인스턴스로 마이그레이션하는 작업에는 주로 다음 단계가 포함됩니다.
...
1. 먼저 Adobe Commerce as a Cloud Service 아키텍처, 확장성 프레임워크 및 상점 기능을 검토합니다.
...
1. 현재 타사 통합을 검토하고 Adobe Commerce as a Cloud Service 플랫폼과의 API 호환성을 확인합니다.

### 구현 단계

다음 단계에서는 마이그레이션의 개발 및 실행 프로세스에 대해 간략히 설명합니다.

1. [Adobe Commerce 관리자](./getting-started.md#create-an-instance)에서 새 Commerce Cloud as a Cloud Service 인스턴스를 만듭니다.

1. 필요한 앱 및 사용자 지정을 설치합니다. Adobe Commerce as a Cloud Service에는 가장 인기 있는 앱 100개가 미리 로드되어 있습니다. 추가 앱 또는 사용자 지정이 필요한 경우 App Builder을 사용하여 이를 다시 구현할 수 있습니다.
...
새 Adobe Commerce as a Cloud Service 인스턴스를 시작하기 전에 유효성을 검사하고 테스트하십시오.
...
새 Adobe Commerce as a Cloud Service 샌드박스 인스턴스를 확인하고 테스트하면 프로덕션 인스턴스를 시작할 수 있습니다.
