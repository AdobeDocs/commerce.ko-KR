...
[!DNL Data Connection] 확장을 설치하면 Commerce _관리자_&#x200B;의 **서비스** 아래의 **시스템** 메뉴에 두 개의 새 구성 페이지가 나타납니다.
...
[!DNL Data Connection] 설정은 전역 값과 웹 사이트 범위 값을 혼합해서 사용합니다. 전역 설정은 전체 Adobe Commerce 인스턴스에 적용됩니다. 웹 사이트 범위 설정을 사용하면 다중 브랜드 및 다중 웹 사이트 판매자가 웹 사이트당 다른 Adobe Experience Platform 샌드박스 및 데이터 세트로 데이터를 라우팅할 수 있습니다.

상점, 백오피스 또는 프로필 데이터 수집을 구성하기 전에 **[!UICONTROL Settings]** 탭의 **[!UICONTROL Scope]** 드롭다운을 구성할 웹 사이트로 설정합니다. 서로 다른 Experience Platform 라우팅이 필요한 각 웹 사이트에 대해 구성을 반복합니다.

| 설정 또는 필드 | 범위 | 메모 |
| --- | --- | --- |
| 조직 ID | 글로벌 | Adobe Commerce 인스턴스당 조직 ID 1개. [Commerce 서비스 커넥터](../landing/saas.md#organizationid)에 이미 구성되어 있습니다. |
| 서비스 계정 자격 증명 | 글로벌 | 서비스 계정 세부 정보를 한 번 입력합니다. 배포가 별도로 필요하지 않는 한 모든 웹 사이트에 대한 동일한 자격 증명으로 Experience Platform API 호출을 인증합니다. |
| 샌드박스 이름 | 웹 사이트 | 범위에서 웹 사이트에 대한 Experience Platform 샌드박스를 선택합니다. [!DNL Data Connection] 백 오피스 이벤트에 대한 기본 웹 사이트 범위 설정입니다. |
| **[!UICONTROL Test connection]** | 웹 사이트 | 현재 **범위**&#x200B;에서 선택한 웹 사이트에 대한 서비스 계정 자격 증명과 샌드박스의 유효성을 검사합니다. |
| 데이터 스트림 ID | 웹 사이트 | 선택한 웹 사이트에 대한 행동 및 백오피스 이벤트 데이터를 라우팅합니다. |
| 데이터 세트 ID | 웹 사이트 | 선택한 웹 사이트에 대한 Commerce 데이터를 저장하는 데이터 세트를 식별합니다. |
| 상점 이벤트, 백 오피스 이벤트, 고객 프로필 | 웹 사이트 | 데이터 수집 옵션은 **범위**&#x200B;에서 선택한 웹 사이트에 적용됩니다. |

#### 다중 웹 사이트 예 {#multi-website-example}

판매자는 하나의 Adobe Commerce 인스턴스에서 두 개의 웹 사이트를 운영합니다.

- **웹 사이트 A(브랜드 A)**&#x200B;에서 **프로덕션** Experience Platform 샌드박스로 상점 및 백오피스 데이터를 보냅니다.
- **웹 사이트 B(브랜드 B)**&#x200B;가 테스트를 위해 **개발** Experience Platform 샌드박스로 데이터를 보냅니다.

판매자가 **[!UICONTROL Service Account/Credential details]** 페이지에서 서비스 계정 자격 증명을 한 번 입력합니다. **[!UICONTROL Settings]** 탭에서 판매자는 **[!UICONTROL Scope]**&#x200B;을(를) **웹 사이트 A**(으)로 설정하고 프로덕션 샌드박스 이름을 선택한 다음 저장합니다. 그런 다음 판매자는 **[!UICONTROL Scope]**&#x200B;을(를) **웹 사이트 B**(으)로 설정하고 개발 샌드박스 이름을 선택하고 저장합니다. 각 웹 사이트에 대해 **[!UICONTROL Test connection]**&#x200B;을(를) 실행하여 올바른 샌드박스 및 자격 증명을 확인합니다.

### 서비스 계정 및 자격 증명 세부 정보 추가

[주문 내역 데이터](#send-historical-order-data) 또는 [고객 프로필 데이터](#send-customer-profile-data)를 수집하여 전송하려는 경우 서비스 계정 및 자격 증명 세부 정보를 추가해야 합니다. 또한 [Audience Activation](https://experienceleague.adobe.com/ko/docs/commerce-admin/customers/audience-activation) 확장을 구성하는 경우 다음 단계를 완료해야 합니다.
...
프로젝트를 만들려면 [Experience Platform API 인증 및 액세스](https://experienceleague.adobe.com/ko/docs/experience-platform/landing/platform-apis/api-authentication) 자습서에 설명된 단계를 따르십시오.

튜토리얼을 진행할 때 프로젝트에 다음과 같은 사항이 있는지 확인합니다.

- 다음 [제품 프로필](https://experienceleague.adobe.com/ko/docs/experience-platform/landing/platform-apis/api-authentication#select-product-profiles)에 액세스: **기본 프로덕션 모든 액세스** 및 **AEP 기본 모든 액세스**.
- 올바른 [역할 및 권한이 구성되었습니다](https://experienceleague.adobe.com/ko/docs/experience-platform/landing/platform-apis/api-authentication#assign-api-to-a-role).
...
1. **Adobe Developer 인증 유형** 메뉴에서 구현한 서버 간 인증 방법을 선택합니다. Adobe에서는 OAuth를 사용하는 것이 좋습니다. [자세히 알아보기](https://developer.adobe.com/commerce/webapi/rest/authentication/server-to-server/)
...
1. **[!UICONTROL Save Config]**&#x200B;을(를) 클릭합니다.
   ...
   저장한 후 **[!UICONTROL Settings]** 탭에서 **[!UICONTROL Sandbox name]** 및 **[!UICONTROL Test connection]**&#x200B;을(를) 포함하여 웹 사이트 범위 구성을 완료합니다. [일반](#general)을 참조하세요.
...
1. **[!UICONTROL General]**&#x200B;의 **[!UICONTROL Settings]** 탭에서 [Commerce 서비스 커넥터](../landing/saas.md#organizationid)에 구성된 대로 Adobe Experience Platform 계정과 연결된 ID를 확인합니다. 조직 ID는 글로벌입니다. Adobe Commerce 인스턴스당 하나의 조직 ID만 연결할 수 있습니다.

1. **[!UICONTROL Scope]** 드롭다운에서 컨텍스트를 **웹 사이트**(으)로 설정합니다. 웹 사이트별로 전역 설정이 적용되는 [구성 범위](#configuration-scope)를 참조하세요.

1. 범위에 웹 사이트의 **[!UICONTROL Sandbox name]**&#x200B;을(를) 입력하십시오.
...
1. (선택 사항) 이미 사이트에 [AEP Web SDK(alloy)](https://experienceleague.adobe.com/ko/docs/experience-platform/collection/js/js-overview)을 배포한 경우 확인란을 활성화하고 AEP Web SDK의 이름을 추가합니다. 그렇지 않으면 이러한 필드를 비워 두고 [!DNL Data Connection] 확장에서 자동으로 배포합니다.
...
[상점](events.md#storefront-events), [사무실](events-backoffice.md) 및 [프로필 레코드](events-profilerecord.md) 데이터에 대한 자세한 내용은 이벤트 항목을 참조하세요. 시계열 [프로필 이벤트](events-backoffice.md#customer-profile-events)의 경우 백 오피스 이벤트 참조를 참조하십시오.
...
1. 주문이 주문, 취소, 환불 또는 배송된 경우와 같은 주문 상태 정보를 보내려면 **사무실 뒷면 이벤트**&#x200B;를 선택하십시오.
   ...
   >[!NOTE]
   >
   >**이전 사무실 이벤트**&#x200B;를 선택하면 모든 이전 사무실 데이터가 Experience Platform Edge로 전송됩니다. 쇼핑객이 데이터 수집을 옵트아웃하는 경우 Experience Platform에서 쇼핑객의 개인 정보 보호 기본 설정을 명시적으로 설정해야 합니다. 이는 컬렉터가 이미 구매자 환경 설정에 따라 동의를 처리하는 상점 이벤트와는 다릅니다. Experience Platform에서 쇼핑객 개인 정보 기본 설정을 설정하는 방법에 대해 [자세히](https://experienceleague.adobe.com/ko/docs/experience-platform/landing/governance-privacy-security/consent/adobe/dataset)알아보세요.

1. (나만의 AEP Web SDK을 사용하는 경우 이 단계를 건너뜁니다.) Adobe Experience Platform에서 데이터 스트림을 [만들기](https://experienceleague.adobe.com/ko/docs/experience-platform/datastreams/configure#create)하거나 컬렉션에 사용할 기존 데이터 스트림을 선택하십시오. **데이터 스트림 ID** 필드에 해당 데이터 스트림 ID를 입력합니다.
...
1. [cron](https://experienceleague.adobe.com/ko/docs/commerce-admin/systems/tools/cron) 작업에 따라 일정에 따라 백 오피스 이벤트 데이터를 업데이트하려면 `Sales Orders Feed` 인덱스를 `Update by Schedule`(으)로 변경해야 합니다.
   ...
   1. 백 오피스 데이터를 처음 활성화하는 경우 다음 명령을 실행하여 다시 인덱싱하고 재동기화를 트리거합니다. [cron](https://experienceleague.adobe.com/ko/docs/commerce-admin/systems/tools/cron) 작업이 올바르게 설정된 경우 이후의 재동기화가 자동으로 수행됩니다.
...
| 필드 | 설명 |
|--- |--- |
| 범위 | 웹 사이트 범위 설정이 적용되는 웹 사이트. 범위를 전환하여 각 웹 사이트에 대한 샌드박스 이름, 데이터 스트림 ID, 데이터 세트 ID 및 데이터 수집 옵션을 구성합니다. |
| 조직 ID (전역) | Adobe DX 제품을 구입한 조직의 ID입니다. 이 ID는 Adobe Commerce 인스턴스를 Adobe Experience Platform에 연결합니다. 전체적으로 구성되었으며 웹 사이트별로 설정되지 않음. |
| 샌드박스 이름(웹 사이트) | Commerce이 선택한 웹 사이트에 대한 데이터를 전송하는 Experience Platform 샌드박스 이름입니다. 다중 브랜드 또는 다중 웹 사이트 배포가 데이터를 다른 샌드박스로 라우팅하는 경우 각 웹 사이트에 대해 별도로 구성합니다. |
...
| **[!UICONTROL Test connection]** | 현재 **범위**&#x200B;에서 선택한 웹 사이트의 서비스 계정 자격 증명과 샌드박스 이름을 확인합니다. 자격 증명을 구성하고 웹 사이트에 대한 샌드박스 이름 또는 범위를 변경한 후 테스트를 실행합니다. |

확장을 구성하면 상점 데이터가 Experience Platform 에지로 흐르기 시작합니다. 백오피스 데이터가 가장자리에 표시되는 데 약 5분이 소요됩니다. cron 일정에 따라 에지에서 후속 업데이트가 표시됩니다.

### 고객 프로필 데이터 보내기

Experience Platform에 보낼 수 있는 프로필 데이터에는 프로필 레코드와 시계열 프로필 이벤트의 두 가지 유형이 있습니다.

프로필 레코드에는 쇼핑객이 Commerce 인스턴스에서 쇼핑객 이름과 같은 프로필을 만들 때 저장되는 데이터가 포함됩니다. 스키마와 데이터 세트가 [올바르게 구성](profile-data.md)되면 프로필 레코드가 Experience Platform으로 전송되고 Adobe의 프로필 관리 및 세분화 서비스 [Real-Time CDP](https://experienceleague.adobe.com/ko/docs/experience-platform/rtcdp/intro/rtcdp-intro/overview)로 전달됩니다.
...
Adobe Commerce은 최대 5년간의 [내역 주문 데이터 및 상태](events-backoffice.md)를 수집합니다. [!DNL Data Connection] 확장을 사용하여 해당 내역 데이터를 Experience Platform으로 보내어 고객 프로필을 보강하고 이전 주문을 기반으로 고객 경험을 개인화할 수 있습니다. 데이터는 Experience Platform 내의 데이터 세트에 저장됩니다.
...
1. [RabbitMQ를 사용](https://experienceleague.adobe.com/ko/docs/commerce-on-cloud/user-guide/configure/service/rabbitmq)합니다.

   >[!NOTE]
   >
   >RabbitMQ는 Commerce 버전 2.4.7 이상에 대해 이미 설정되었지만 소비자를 활성화해야 합니다.

1. `CRON_CONSUMERS_RUNNER` 환경 변수를 사용하여 `.magento.env.yaml`에서 cron 작업별로 메시지 큐 소비자를 사용하도록 설정하십시오.
   ...
   >[!NOTE]
   >
   >사용 가능한 모든 구성 옵션에 대해 알아보려면 [변수 배포 설명서](https://experienceleague.adobe.com/ko/docs/commerce-on-cloud/user-guide/configure/env/stage/variables-deploy#cron_consumers_runner)를 참조하세요.
...
>[!IMPORTANT]
>
>**데이터 수집** 탭에서 [지정한](#data-collection) 데이터 스트림 ID가 사용자 지정 특성을 수집하기 위해 스키마에 연결된 ID와 일치하는지 확인하십시오.
...
- Experience Platform의 스키마에서 특성 이름이 업데이트된 경우 Experience Platform 스키마 변경을 트리거하려면 **데이터 사용자 지정** 탭에 구성을 저장해야 합니다. 이 변경 사항은 **[!UICONTROL Refresh]** 단추를 클릭할 때 **사용자 지정 순서 특성** 표에 반영됩니다.
...
Commerce 스토어에서 데이터가 수집되고 있는지 확인하려면 [Adobe Experience Platform 디버거](https://experienceleague.adobe.com/ko/docs/experience-platform/debugger/home)를 사용하여 Commerce 사이트를 검사하십시오. 데이터가 수집되고 있는지 확인한 후 구성 중에 만든 [데이터 집합에서 데이터를 반환하는 쿼리를 실행하여 상점 및 백 오피스 이벤트 데이터가 가장자리에 표시되는지 확인할 수 있습니다](overview.md#enable-extension).
...
결과가 예상과 다른 경우 데이터 세트를 열고 실패한 일괄 처리 가져오기를 찾습니다. [일괄 가져오기 문제 해결](https://experienceleague.adobe.com/ko/docs/experience-platform/ingestion/batch/troubleshooting)에 대해 자세히 알아보세요.

### 프로필 데이터가 Experience Platform에 표시되는지 확인

문제 해결 제안은 [고객 프로필 데이터 보내기](#send-customer-profile-data)의 팁을 참조하십시오.
