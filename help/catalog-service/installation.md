...
[카탈로그 서비스 GraphQL API](https://developer.adobe.com/commerce/webapi/graphql/schema/catalog-service/)를 사용하여 Commerce 인스턴스에서 제품 데이터를 요청하고 받으려면 카탈로그 서비스를 설치하십시오. 카탈로그 서비스는 repo.magento.com 저장소에서 작성기 PHP 메타패키지로 제공됩니다.
...
샌드박스 엔드포인트에서 모든 로드 테스트를 수행합니다. 부하 테스트를 시작하기 전에 [지원 티켓](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-customer-support-experience#create-a-support-ticket-with-experience-league)을 제출하여 서비스 팀이 추가 서버 트래픽을 예측할 수 있도록 합니다.
...
1. 작성기를 사용하여 `magento/catalog-service` 모듈 추가:
...
1. 작성기를 사용하여 `magento/catalog-service` 패키지 추가:
...
[!DNL Catalog Service]을(를) 설치한 후 카탈로그 서비스를 Adobe Commerce 인스턴스와 통합하려면 다음 작업을 완료하십시오. 이 통합을 통해 Commerce 인스턴스, 카탈로그 서비스 및 기타 지원 서비스 간에 데이터를 동기화하고 통신할 수 있습니다. [SaaS 데이터 내보내기 확장](../data-export/overview.md)에서 데이터 동기화를 처리합니다.
...
1. [데이터 관리 대시보드](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard)에서 초기 데이터 동기화를 수행합니다.
