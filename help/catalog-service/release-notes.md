...
![새로 만들기](../assets/new.svg) **준비된 범주 URL 키 동기화**—카탈로그 준비 데이터 내보내기 모듈(`magento/module-catalog-staging-data-exporter`)을 포함하도록 카탈로그 서비스 메타패키지 종속성을 업데이트했습니다. 이 모듈은 단계적 범주 `url_key` 변경이 적용되면 제품 피드를 다시 내보내 단계적 카탈로그 변경 내용이 SaaS 카탈로그(카탈로그 서비스, 라이브 검색 및 제품 권장 사항)에 올바르게 전파됩니다.
