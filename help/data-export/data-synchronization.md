...
SaaS 데이터 내보내기는 데이터를 최신 상태로 유지하기 위해 제품 데이터를 Adobe Commerce 인스턴스에서 Commerce 서비스 플랫폼으로 지속적으로 이동합니다. 예를 들어 제품 권장 사항에는 현재 카탈로그 정보가 있어야 정확한 이름, 가격 및 가용성의 권장 사항을 정확하게 반환할 수 있습니다. 동기화 프로세스 모니터링에 대한 자세한 내용은 [동기화 프로세스 보기 및 관리](#view-and-manage-the-synchronization-process)를 참조하십시오.
...
[!DNL Adobe Commerce Optimizer Connector]개 피드에 대해 동일한 부분 동기화 cron 작업이 실행됩니다. 커넥터별 제출 및 오류 처리에 대해서는 [커넥터 동기화 파이프라인](../aco-connector/connector-sync-pipeline.md)을 참조하십시오.
...
>[!NOTE]
>
>데이터 관리 대시보드는 라이브 검색, 제품 권장 사항 또는 카탈로그 서비스가 설치된 경우에만 사용할 수 있습니다. 이러한 서비스가 있거나 [Adobe Commerce Optimizer 커넥터](../aco-connector/overview.md)가 설치된 경우 데이터 피드 동기화 상태 대시보드를 사용할 수 있습니다. 범위 제어 및 제출 오류를 포함한 Optimizer 커넥터 파이프라인 동작에 대해서는 [커넥터 동기화 파이프라인](../aco-connector/connector-sync-pipeline.md)을(를) 참조하십시오.
