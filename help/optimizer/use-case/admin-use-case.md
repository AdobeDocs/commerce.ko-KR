...
- [!DNL Adobe Commerce Optimizer] 카탈로그 보기 및 정책을 사용하여 사용자 지정 카탈로그 보기 및 데이터 액세스 필터를 만들고 Edge Delivery에서 제공하는 Adobe Commerce 상점으로 데이터를 보내는 방법에 대해 알아봅니다.
...
## &#x200B;1. [!DNL Adobe Commerce Optimizer] 인스턴스 액세스
...
1. 방금 만든 필터를 활성화하려면 작업 점(...)을 클릭합니다. **사용**&#x200B;을 선택하세요.
...
1. 왼쪽 레일에서 _스토어 설정_ 섹션을 확장하고 **[!UICONTROL Catalog views]**&#x200B;을(를) 클릭합니다.
   ...
   기존 카탈로그 보기 개수: *Arkbridge*, *Kingsbluff* 및 *모든 보기*.
...
1. Storefront 구성 파일(`config.json`)을 업데이트해 보십시오.
   ...
   이 실험에서는 [!DNL Adobe Commerce Optimizer]의 유연성을 보여 줍니다. 카탈로그 데이터를 복제하지 않고도 다양한 카탈로그 보기와 가격 책자 사이를 빠르게 전환하여 다양한 대상자를 위한 사용자 지정 쇼핑 경험을 만들 수 있습니다.
...
- **해결 방법:** 가격 장부 ID가 [!DNL Adobe Commerce Optimizer] 인스턴스에서 사용할 수 있는 ID와 일치하는지 확인하십시오.

**문제:** 검색 결과 없음

- **해결 방법:** 카탈로그 보기 정책에서 검색된 제품 범주를 허용하는지 확인합니다.

추가 도움말은 [[!DNL Adobe Commerce Optimizer] 설명서](../overview.md)를 참조하거나 Adobe 지원에 문의하십시오.
...
[!DNL Adobe Commerce Optimizer]에 대해 계속 배우려면:
