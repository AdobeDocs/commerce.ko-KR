...
▢ 프로덕션 데이터가 인스턴스에 로드됩니다.
▢ [GraphQL 끝점](https://developer.adobe.com/commerce/webapi/graphql/)을 알고 있습니다.
▢ 인스턴스가 [출시 준비](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/launch/checklist) 요구 사항을 충족합니다.

## Commerce Optimizer 인스턴스 확인 {#verify-optimizer}

[!DNL Adobe Commerce Optimizer] 프로덕션 인스턴스가 올바르게 설정되어 있는지 확인하십시오.

▢ 프로덕션 인스턴스가 활성 상태입니다. 프로비저닝 방법은 [시작하기](../get-started.md)를 참조하십시오.
▢ 인스턴스가 올바른 영역에 있습니다.
▢ 환경 유형은 프로덕션입니다.
▢ 조직 ID, 클라이언트 ID, 수집 URL 및 Commerce Optimizer URL을 알고 있습니다. [시작하기](../get-started.md)를 참조하십시오.
▢ 구성된 제한 및 경계는 Adobe Customer Technical Advisor(CTA)에서 확인한 값과 일치합니다.
...
▢ [Facet](../merchandising/facets/overview.md)이(가) 구성되었습니다.
▢ [동의어](../merchandising/synonyms/overview.md)가 구성되었습니다.
▢ [머천다이징 규칙](../merchandising/rules/overview.md)이 구성되었습니다.
▢ [가격 팩시팅 및 검색 언어](../settings.md)는 해당 기능을 사용할 때 요구 사항과 일치합니다.
...
▢ `commerce-endpoint`은(는) 프로덕션 Commerce Optimizer GraphQL 엔드포인트 또는 API Mesh 사용 시 API Mesh 엔드포인트를 가리킵니다.
▢ `headers.cs.AC-view-ID`에는 프로덕션 Commerce Optimizer 인스턴스의 카탈로그 보기 ID가 있습니다.
...
▢ **아키텍처:** 호스트 상거래 시스템은 결제, 배송, 세금 및 인벤토리를 처리합니다([!DNL Adobe Commerce Optimizer] 아님).
▢ **아키텍처:** API Mesh 및 App Builder이 호스트 상거래 시스템과 [!DNL Adobe Commerce Optimizer] 간에 동기화 상태를 유지합니다.
▢ **전자 메일:** 트랜잭션 전자 메일 배달이 작동합니다(주문 확인, 배송 등).
...
`.aem.live` 도메인에서 ▢ 콘텐츠 및 디자인 QA가 완료되었습니다.
▢ favicon이 사이트에서 올바르게 구성되고 제공됩니다.
▢ da.live 및 제품 비주얼은 [구성된](https://docs.da.live/administrators/guides/permissions) 전용 자격 증명을 사용합니다.
▢ 드롭인(장바구니, 체크아웃, PDP, PLP, 인증, 계정)은 [사용자 지정](../storefront.md)되며 테스트됩니다.
▢ 상점 브랜딩은 CSS 디자인 토큰, 타이포그래피 및 색상을 반영합니다.

### SEO 및 색인 지정

메타데이터, URL 및 크롤링 동작을 확인합니다.

▢ 문서 제목 메타데이터가 키 페이지(특히 PDP 및 PLP)에 대해 있습니다. _Adobe Commerce 상점_ 설명서에서 [SEO 메타데이터](https://experienceleague.adobe.com/developer/commerce/storefront/setup/seo/metadata/){target="_blank"}를 참조하십시오.
▢개의 PDP에 [메타데이터 및 구조화된 데이터](https://experienceleague.adobe.com/developer/commerce/storefront/setup/seo/metadata/){target="_blank"}(예: JSON-LD)가 포함됩니다.
▢ 제품 URL 형식이 일관됩니다(예: `domain/product-name`).
▢ 가상 URL이 표준 URL로 리디렉션됩니다.
▢ 프로젝트에는 인덱싱을 허용하는 `robots.txt`이(가) 포함되어 있습니다. 필요한 경우 인덱싱을 허용하고 사이트 맵을 참조하며 인덱싱하지 않을 경로를 차단합니다(예: `/drafts`).
▢ [리디렉션](https://www.aem.live/docs/redirects) 파일은 마이그레이션에서 URL 변경 사항을 다룹니다(예: `.html` 제거 후).
▢ 사이트 맵이 존재하며 필요에 따라 Google 검색 콘솔에 제출됩니다.
▢개의 정식 URL이 `3xx` 또는 `4xx`이(가) 아닌 `2xx` 상태를 반환합니다.
▢개의 다국어 사이트가 사이트 맵에 `hreflang`개의 태그를 포함합니다.
▢ Google 검색 콘솔 검사 보고서는 최신 상태입니다.

### 사전 렌더링

활성화된 서버측 렌더링을 확인합니다.

키 페이지에 대해 ▢ 미리 렌더링이 설정되어 있습니다. _AEM 상점_ 설명서에서 [Adobe Commerce을 위한 사전 렌더링](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/aem-prerender/){target="_blank"}을 참조하십시오.
▢ URL은 소문자를 사용하므로 사전 렌더링으로 인해 링크가 끊어지지 않습니다.
...
▢ 스토어프론트는 _Adobe Commerce 스토어프론트_ 설명서에서 [성능 모범 사례](https://experienceleague.adobe.com/developer/commerce/storefront/get-started/performance/){target="_blank"}를 따릅니다.
▢(선택 사항) Google Analytics 및 Google Tag Manager가 구성되어 있습니다.
▢ [Storefront 이벤트](https://github.com/adobe/commerce-events/tree/main/examples/events/snowplow-debugger) 구현이 유효하며 데이터가 Adobe Commerce *관리자*&#x200B;의 [!DNL Live Search] 및 [!DNL Product Recommendations] 대시보드에 표시됩니다.
▢ [Commerce 구성](https://experienceleague.adobe.com/developer/commerce/storefront/setup/configuration/commerce-configuration/){target="_blank"}의 `environment` analytics 매개 변수는 개발 중 `"Testing"`이고 Go-Live 시 `"Production"`입니다. [Analytics 계측](https://experienceleague.adobe.com/developer/commerce/storefront/setup/analytics/instrumentation/){target="_blank"}을 참조하십시오.
▢ 등대 점수는 이 항목의 지침에 따라 타겟을 충족합니다(예: 주요 페이지의 `100`).

### 보안 및 액세스

권한 및 비밀을 확인합니다.

DA 콘텐츠 및 EDS 사이트에 대해 적절한 권한이 ▢개 구성되었습니다. [DA.live 권한](https://da.live/docs/administration/permissions) 및 [작성을 위한 인증 설정](https://www.aem.live/docs/authentication-setup-authoring)을 참조하세요.
▢ 제품 시각적 개체 통합이 프로비저닝되었습니다. [AEM Cloud Service 액세스 개요](https://experienceleague.adobe.com/en/docs/experience-manager-learn/cloud-service/accessing/overview#)를 참조하십시오.
이메일 템플릿의 암호 재설정 링크 ▢개가 Edge Delivery Services 설정과 일치합니다. 상점 FAQ를 참조하십시오. [Edge Delivery Services 또는 Helix로 마이그레이션한 후 이메일 템플릿 링크가 끊어진 경우 어떻게 해야 합니까?](https://experienceleague.adobe.com/developer/commerce/storefront/troubleshooting/faq/#what-should-i-do-if-my-email-template-links-are-broken-after-migrating-to-edge-delivery-services-or-helix){target="_blank"}.
...
▢ Adobe Commerce Fastly를 사용하는 경우 CDN 제거 토큰을 사용할 수 있으며 [사이트 구성](https://tools.aem.live/tools/cdn-setup/index.html)에 `authToken` 및 `serviceId`이(가) 포함됩니다.
...
▢ [다중 저장소 설정](https://experienceleague.adobe.com/developer/commerce/storefront/setup/seo/indexing/#multi-store-setups){target="_blank"}의 경우 카탈로그 서비스 및 [!DNL Live Search] 요청에는 저장소별 캐시 버스터(예: 쿼리 매개 변수 또는 CDN 규칙)가 포함됩니다.
▢ 푸시 무효화가 처음부터 끝까지 작동합니다(변경 사항을 게시한 다음 프로덕션 도메인에서 확인).
전환 전 ▢ DNS TTL이 충분히 낮습니다.
▢ DNS A 및 CNAME 레코드가 모든 도메인 및 호스트 이름에 대해 올바릅니다.
▢ 프로덕션 도메인에 대해 SSL/TLS 인증서가 프로비저닝되고 확인되었습니다.
▢ `www` 및 apex 리디렉션이 올바르게 작동합니다.
...
▢ **액세스:** 모든 *관리자* 사용자에 대해 2단계 인증이 활성화되어 있습니다.
▢ **액세스:** 프로젝트와 연결된 비활성 또는 사용하지 않는 관리자 사용자가 없습니다.
▢ **방화벽:** 웹 응용 프로그램 방화벽(WAF)이 구성 및 확인되었습니다.
▢ **PCI:** 프로덕션에 대한 보안 침투 테스트(PCI 범위)가 완료되었습니다.
▢ **검사:** Adobe 보안 검사 도구가 등록되어 초기 검사가 완료되었습니다.
▢ **액세스:** CORS는 승인된 원본만 허용합니다.
▢ **준수:** [!DNL Adobe Commerce Optimizer]에 대한 [공유 책임 모델](../shared-responsibility.md)이 최신 상태이며 Adobe과 고객 책임을 명확하게 정의합니다.
...
▢ **UAT:** 스테이징 및 프로덕션에서 사용자 수락 테스트가 완료되었습니다.
▢ **성능:** 부하 및 스트레스 테스트가 완료되었으며 Adobe CTA 또는 CSE에서 결과를 얻었습니다.
...
▢ **지원:** 귀하의 팀은 P1 핫라인으로 전화하는 **이전** 지원 티켓을 열도록 교육되었습니다.
▢ **출시 이후:** 프로덕션 도메인에서 Lighthouse 점수를 확인하십시오.
▢ **시작 후:** Google 콘솔 모니터링에서 색인 지정 및 크롤링 검색 오류 모니터링
▢ **시작 이후:** 모니터 404에서 트래픽이 많은 레거시 URL에 대해 보고하고 리디렉션을 추가합니다.
▢ **게시물 실행:** 프로덕션에서 MarTech 및 Analytics 데이터를 확인합니다.
▢ **출시 이후:** CTA, CSE 또는 AM에 SLA의 높은 모니터링을 활성화하도록 요청하십시오.
▢ 재해 복구 계획이 있으며 테스트를 통과했습니다.
▢ 보일러플레이트 및 확장 패키지를 추적하여 현재 버전으로 업그레이드하는 프로세스가 마련되어 있습니다.
