...
카탈로그 레이어를 사용하면 원본 소스 데이터를 변경하지 않고 제품 데이터를 수정할 수 있습니다. 레이어는 기본 카탈로그 위에 레이어를 만들어 이름, 설명, 이미지, 링크 및 메타데이터와 같은 제품 속성을 수정합니다. 원본 제품 데이터는 그대로 유지되므로 언제든지 제품을 안전하게 사용자 정의하고 변경 사항을 되돌릴 수 있습니다.

![카탈로그 레이어](../assets/catalog-layers.png)

## 카탈로그 레이어 작동 방식

고객이 상점을 볼 때 시스템은 기본 카탈로그 데이터를 활성 카탈로그 레이어와 결합하여 최종 제품 정보를 표시합니다. 프로세스는 다음과 같이 작동합니다.
...
1. **필드 처리**—다른 필드 형식이 다르게 처리됩니다.
   ...
   * **필드 재정의** - 이름, 설명 및 메타 제목과 같은 텍스트 필드가 레이어 값으로 대체되며 우선 순위가 높은 레이어가 우선합니다.
...
>[!CAUTION]
>
> 카탈로그 보기에 AEM-Assets 레이어를 할당합니다. 레이어가 할당되지 않으면 제품 이미지 데이터가 예기치 않게 덮어쓰여집니다.
...
1. **[!UICONTROL Store setup]**(으)로 이동한 다음 **[!UICONTROL Catalog views]**&#x200B;을(를) 클릭합니다.

1. 레이어를 적용할 카탈로그 보기를 선택합니다.

1. 카탈로그 레이어 섹션에서 **AEM-Assets** 레이어를 찾습니다.

1. 이 카탈로그 보기에 대한 레이어를 활성화하려면 활성화합니다.
...
카탈로그 보기 UI를 사용하면 수동으로 레이어를 만들고 관리할 수 있습니다. 이는 AI 기반 권장 사항을 생성하는 Sites Optimizer과 같은 통합에 특히 유용합니다.

>[!NOTE]
>
>Sites Optimizer 레이어가 카탈로그 보기에 없는 경우 Sites Optimizer의 자동 수정 기능에서 자동으로 레이어를 만들고 가장 높은 우선 순위(가장 높은 번호)를 지정합니다. 이 레이어를 삭제하면 다음에 Sites Optimizer의 자동 수정 기능이 실행되고 기존 레이어가 더 낮은 순서 번호로 이동될 때 다시 생성됩니다. Sites Optimizer 레이어가 다른 주문 번호에 이미 있는 경우 자동 수정 기능은 해당 우선 순위를 변경하지 않습니다.
...
1. **[!UICONTROL Store setup]** > **[!UICONTROL Catalog views]**(으)로 이동합니다.
...
1. 레이어를 만들려면 **[!UICONTROL Save]**&#x200B;을(를) 클릭합니다.
...
* 숫자가 높을수록 우선 순위가 높고 다른 모든 레이어를 오버라이드합니다.
...
1. **[!UICONTROL Save]**&#x200B;을(를) 클릭하여 새 우선 순위를 적용합니다.

>[!IMPORTANT]
>
>레이어 우선 순위 변경 사항은 즉시 적용되며 고객이 상점에서 보는 내용에 영향을 줍니다. 저장하기 전에 미리 보기를 검토하여 올바른 값이 적용되었는지 확인합니다(**미리 보기를 아직 사용할 수 없음**).

## 우수 사례

카탈로그 레이어를 사용하여 작업할 때 다음 권장 사항을 따르십시오.

* **설명적인 이름 사용**—이름 레이어를 명확하게 지정하여 용도를 나타내십시오(예: &quot;Holiday 2025 Campaign&quot; 또는 &quot;SEO 최적화 — 제품 페이지&quot;).

* **레이어 제한** - 시스템이 여러 레이어를 지원하지만 너무 많이 사용하면 성능에 영향을 줄 수 있습니다. 가능한 경우 레이어를 통합합니다.


* **문서 우선 순위 논리** - 의도하지 않은 재정의를 방지하기 위해 우선 순위가 높은 레이어를 추적합니다.

* **Sites Optimizer 레이어 검토** - Sites Optimizer에서 자동 고침을 사용하면 시스템에서 가장 높은 우선 순위의 레이어를 만듭니다. AI 권장 사항을 오버라이드하는 수동 레이어를 추가할 때 주의하십시오. [Sites Optimizer](../manage-results/opportunities.md) 사용에 대해 자세히 알아보세요.

* **성능 모니터링**—제품 페이지가 느리게 로드되는 경우 레이어 구성을 검토하고 레이어를 통합하는 것이 좋습니다.

## 다음과 같음

* [카탈로그 보기](catalog-view.md) - 다른 상점 여정에 대한 카탈로그 보기 구성
* [제품 시각화](product-visuals.md) - 제품 이미지에 AEM Assets 사용
* [기회](../manage-results/opportunities.md) - 카탈로그 레이어를 사용한 AI 기반 최적화에 대해 알아봅니다.
* [제한된 액세스 키](restricted-access-keys.md)—서명 토큰 인증을 사용하여 카탈로그 보기를 보호합니다.
