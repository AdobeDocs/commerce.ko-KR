...
![[!DNL Composable Catalog Data Model] 아키텍처](assets/ccdm-architecture.png)
...
CCDM을 구성하는 개념에 대한 자세한 내용은 다음 섹션을 참조하십시오.
...
![[!DNL Composable Catalog Data Model] 제품 컨텍스트 식별자](assets/ccdm-product-id.png)
...
| **새 카탈로그 제품 범위**: 채널, 정책 및 범위는 CCDM에서 도입한 새 제품 범위입니다. 이러한 제품 범위는 storefront 서비스 레이어의 웹 사이트, 스토어 및 스토뷰 범위를 대체합니다. [자세히 알아보기](#product-context-management). | <ul><li>CCDM은 새로운 범위를 통해 다중 지역, 다중 비즈니스 단위, 다중 브랜드 및 다국어 사용 사례로 확장하는 기능을 단일 기본 카탈로그를 사용하여 쉽게 잠금 해제합니다.</li><li>카탈로그 관리에서 데이터 중복을 제거합니다.</li></ul> |
...
## CCDM을 통해 가장 큰 혜택을 받는 판매자는 무엇입니까?

다음 표에는 판매자가 직면하는 일반적인 문제와 CCDM이 이러한 문제를 해결하는 방법이 나와 있습니다.
...
| 자동차/제조 부품 대기업 | <ul><li>자동차 또는 기계 부품을 판매합니다. 그 제품은 모든 고객에게 동일합니다.</li><li>다른 딜러가 고객에게 부품을 판매합니다.</li><li>각 딜러는 고유의 가격, 재고 및 배송 방법을 가지고 있습니다</li></ul> | 배송 통합을 다르게 하려면 각 딜러가 별도의 웹 사이트를 보유하고 있어야 합니다. 그러나 별도의 웹 사이트에서는 일반적인 카탈로그 데이터 모델이 데이터를 복제하도록 합니다. 따라서 미국에 3000개의 딜러가 있다면, 모든 딜러가 동일한 카탈로그를 사용하고 있음에도 불구하고 상인이 3000개의 카탈로그 사본을 만듭니다. 이 데이터 중복은 성능 제한을 방해합니다. CCDM을 사용하면 데이터 중복이 제거됩니다. |
| 포장/물류 회사 | <ul><li>그곳에는 여러 개의 배송 장소가 있습니다</li><li>고객마다 가격이 다릅니다</li><li>고객 2명이 2곳에서 사용할 수 있는 동일한 제품은 4가지 가격이 있습니다</li></ul> | 고객별 가격을 관리하기 위해 고객 그룹을 사용함에도 불구하고, 일반적인 카탈로그 데이터 모델은 위치별 가격을 관리할 수 있는 기능이 없습니다. 또한 판매자는 위치/웹 사이트별로 고유한 가시성 규칙을 원합니다. 이러한 복잡한 가격 및 가시성 규칙에 대한 관리는 CCDM을 통해 규모에 맞게 잠금 해제할 수 있습니다. |

### 여기서 어디로 가야 합니까

- [데이터 수집 API](https://developer-stage.adobe.com/commerce/services/composable-catalog/data-ingestion/using-the-api/)를 사용하여 CCDM으로 데이터를 수집합니다.
...
- 단일 기본 카탈로그를 가진 회사가 CCDM API를 사용하여 제품 데이터를 추가하고, 프로젝션을 사용하여 카탈로그를 정의하고, 헤드리스 상점에 표시할 카탈로그 데이터를 검색하는 방법을 보여 주는 [자습서를 완료합니다](https://developer-stage.adobe.com/commerce/services/composable-catalog/ccdm-use-case/).
