...
>[!NOTE]
>
>Adobe Commerce Optimizer 또는 Adobe Commerce Optimizer 커넥터를 사용하는 Commerce 솔루션의 경우, 카탈로그 서비스 GraphQL API 대신 [머천다이징 서비스 GraphQL API](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/using-the-api/)를 사용하십시오.

## Optimizer에 대한 데이터 동기화 확인 {#aco-data-sync-verification}
...
>[!NOTE]
>
>[!DNL Commerce Storefront on Edge Delivery Services]에 대한 [추가 구성 요소](https://experienceleague.adobe.com/en/tools/commerce-storefront/dropins/all/introduction/)에서 최신 GraphQL 변경 내용(새 필드, 제한 및 쿼리 동작)을 자동으로 선택합니다.
...
## ACCS 샌드박스 전용 실험 기능 {#accs-sandbox-experimental}

>[!IMPORTANT]
>
>이 기능은 실험적이며 [!DNL Adobe Commerce as a Cloud Service]의 샌드박스 환경에서만 사용할 수 있습니다.
>
>이 기능은 예고 없이 변경될 수 있습니다.
...
## AEM Assets 인스턴스 매핑 {#aem-assets-instance-mapping}

>[!NOTE]
>
>[!DNL Adobe Commerce as a Cloud Service]을(를) [!DNL AEM Assets]에 연결할 때 [!DNL AEM Assets] 단계 인스턴스가 샌드박스 [!DNL Adobe Commerce as a Cloud Service] 인스턴스 및 기타 비프로덕션 환경에 매핑됩니다. [!DNL AEM Assets] 프로덕션 인스턴스가 [!DNL Adobe Commerce as a Cloud Service] 프로덕션 인스턴스에 매핑됩니다.

## IMS ID 및 Single Sign-On 정보 {#ims-identity-and-sso-config}
...
| 서비스 | 상점 첫 화면 |
| --- | --- |
| [AEM Assets 통합](../aem-assets-integration/release-notes.md)<br>[Commerce Optimizer 커넥터](../aco-connector/release-notes.md) | [Storefront 릴리스 정보](https://experienceleague.adobe.com/en/tools/commerce-storefront/releases/)<br>[Storefront 변경 로그](https://experienceleague.adobe.com/en/tools/commerce-storefront/releases/changelog/) |
