...
[!DNL Commerce] 라이선스 소유자만 이러한 API 키를 생성할 수 있습니다. 라이선스 소유자가 아닌 경우 스토어에 대한 Commerce 라이선스를 소유한 사람 또는 팀에 키를 요청하십시오.
...
| 서비스 | 가용성 |
| --- | --- |
| Adobe AI 제공 [[!DNL Product Recommendations]](/help/product-recommendations/overview.md) | Adobe Commerce |
| Adobe AI 제공 [[!DNL Live Search]](/help/live-search/overview.md) | Adobe Commerce |
...
키 식별자 *Production* 및 *Sandbox*&#x200B;은(는) [!DNL Commerce Services]이(가) 데이터를 저장하는 SaaS 데이터 공간 환경을 참조합니다(Adobe Commerce 환경이 아님). 로컬, 개발, 스테이징 및 프로덕션 Adobe Commerce 환경에서 동일한 API 키 세트를 사용할 수 있습니다. 중요한 것은 사용자가 구성하는 데이터 공간에 대한 올바른 키 쌍을 붙여넣는 것입니다.
...
1. _API 키_ 섹션에 이름을 입력하고 **새로 추가**&#x200B;를 클릭하여 대화 상자를 열어 새 키를 다운로드합니다.
   ...
   >[!WARNING]
   >
   > 개인 키는 한 번만 복사하거나 다운로드할 수 있습니다. 안전하게 보관하십시오.

1. **다운로드**&#x200B;를 클릭하여 개인 키를 저장한 다음 대화 상자를 닫습니다.
...
[!DNL Product Recommendations] 및 [!DNL Live Search]의 경우 SaaS 데이터 공간에 카탈로그 및 동작 데이터가 포함되어 있습니다. [!DNL Commerce] 구성에서 [선택하여](https://experienceleague.adobe.com/ko/docs/commerce-admin/config/services/saas) SaaS 데이터 공간으로 [!DNL Commerce] 인스턴스를 지정할 수 있습니다.

>[!WARNING]
>
> 프로덕션 **설치에만**&#x200B;프로덕션 SaaS 데이터 공간[!DNL Commerce]을 사용합니다. 비프로덕션 환경에서 사용하면 테스트 및 라이브 데이터(예: 스테이징 URL 또는 테스트 카탈로그 데이터)를 혼합할 수 있습니다. 이 경우 데이터 정리를 요청하려면 [지원 요청을 제출](https://experienceleague.adobe.com/ko/docs/commerce-knowledge-base/kb/overview)하십시오.

관리에서 라이브 검색 구성 필드를 찾을 수 없는 경우 선택한 데이터 공간에 대해 올바른 API 키 쌍을 입력했는지 확인합니다(프로덕션 데이터 공간은 프로덕션 키를 사용하고 데이터 공간 테스트는 샌드박스 키를 사용). 잘못된 키를 구성하면 해당 Adobe Commerce 환경에서 라이브 검색과 같은 SaaS 서비스를 사용할 수 없습니다.

### API 키 삭제 {#delapikey}

>[!WARNING]
>
>아직 사용 중인 키를 삭제하면 연결된 서비스가 즉시 중단됩니다.
...
비프로덕션 환경에서 테스트 데이터 공간을 사용할 수 있지만 여러 환경에서 동일한 데이터 공간을 동시에 사용하지 마십시오. 테스트 데이터 공간을 다른 환경으로 이동하려면 새 환경에서 선택하고 구성하기 전에 데이터 정리를 수행하십시오.
...
1. _관리자_ 사이드바에서 **시스템** > 서비스 > **Commerce 서비스 커넥터**(으)로 이동합니다.
   ...
   **[!UICONTROL Commerce Services Connector]** 섹션이 표시되지 않으면 원하는 [!DNL Commerce]서비스[[!DNL Commerce] 에 대한 &#x200B;](#availableservices) 모듈을 설치하고 `magento/module-services-id` 패키지가 설치되었는지 확인하십시오.
...
1. _[!UICONTROL Sandbox API Keys]_&#x200B;및_[!UICONTROL Production API Keys]_ 섹션에 키 값을 붙여 넣습니다.
   ...
   데이터베이스 백업이나 스냅샷에서 복사한 키 값을 붙여넣지 마십시오. 구성이 저장되면 추가 암호화 계층이 적용되고 키가 작동하지 않습니다.
...
1. **저장소의 현재 구성에 사용할**&#x200B;데이터 공간[!DNL Commerce]을(를) 선택하십시오.
   ...
   Commerce 서비스와 통합할 인스턴스가 따로 있는 경우 [지원 티켓을 제출](https://experienceleague.adobe.com/ko/docs/commerce-knowledge-base/kb/help-center-guide/magento-help-center-user-guide#submit-ticket)하여 각 추가 인스턴스에 대해 새 SaaS 프로젝트를 요청하세요. 지원에서 SaaS 프로젝트를 만든 후 동일한 API 키를 사용하여 인스턴스에 대한 통합을 구성하고 데이터 공간에 대한 새 SaaS 프로젝트를 선택합니다.
