...
릴리스된 최신 라이브 검색 버전에 대한 지원이 제공됩니다. 이전 버전에 대한 릴리스 노트는 참조를 위해 제공됩니다.
업데이트에는 다음이 포함됩니다.
...
![새로 만들기](../assets/new.svg) [!DNL Live Search]은(는) 이제 의미 체계 검색을 지원하며 [**설정**](./settings.md#semantic-search) 작업 영역에서 구성할 수 있습니다. 키워드 검색과 달리 의미론적 검색은 AI를 사용하여 의미와 컨텍스트별로 제품을 일치시킵니다. 따라서 &quot;겨울 하이킹을 위해 따뜻한 것&quot;과 같은 자연어 쿼리에 대한 검색 결과가 없는 것을 줄일 수 있습니다. [자세히 알아보기](./semantic-search.md)
...
- Adobe Commerce on Cloud 및 온-프레미스 — 수동으로 활성화해야 합니다.
...
![수정](../assets/fix.svg) 결과 페이지의 맨 위에 [고정된 제품](./facets-manage.md#pinunpin-facet)이 나열되지 않는 문제를 해결했습니다.
...
![수정](../assets/fix.svg) &quot;검색 규칙&quot; 탭의 이름이 &quot;검색 머천다이징&quot;으로 변경되었습니다.
...
이제 ![신규](../assets/new.svg) 사용자가 지능형 순위 규칙을 하위 범주에 추가할 수 있습니다.
...
![신규](../assets/new.svg) 사용자는 이제 규칙을 추가할 때 범주 이름으로 검색할 수 있습니다.
![새로 만들기](../assets/new.svg) 이제 범주 트리 보기를 사용하여 규칙이 적용된 범주를 볼 수 있습니다.
![새로 만들기](../assets/new.svg) 범주 미리 보기에는 선택한 범주만 표시됩니다.
![신규](../assets/new.svg) AEM CIF [팝오버 위젯](https://github.com/adobe/aem-cif-guides-venia/pull/319) 및 [PLP 위젯](https://github.com/adobe/aem-cif-guides-venia/pull/320) 구성 요소를 통해 AEM 사이트에서 [!DNL Live Search]을(를) 이용할 수 있습니다.
...
- 14일
- 30일
![수정](../assets/fix.svg) &#39;이벤트&#39;(Boost/Pin/Bury/Hide)의 이름이 &#39;수동 순위&#39;로 변경되었습니다.
...
![새로 만들기](../assets/new.svg) 실시간 검색은 이제 관리자의 &#39;재고 부족 제품 표시&#39; 설정에 의한 필터링을 지원합니다. &#39;재고 부족 제품 표시&#39;가 false로 설정되어 있으면 `inStock = true`이(가) 필터에 추가됩니다.
...
![새로 만들기](../assets/new.svg) [[!DNL storefront popover]](overview.md)에서 사용 가능한 줄 수는 *관리자*에서 구성할 수 있습니다.
[!DNL Live Search]에 대해 ![새로 만들기](../assets/new.svg) Beta [PWA](https://developer.adobe.com/commerce/pwa-studio/)이(가) 지원됩니다.
...
![수정](../assets/fix.svg) [[!DNL Storefront popover]](storefront-popover.md)은(는) 사용 가능한 경우 이제 [특별 가격](https://experienceleague.adobe.com/ko/docs/commerce-admin/catalog/products/pricing/product-price-special)&#x200B;(최소 최종 가격)을 표시합니다.
...
![수정](../assets/fix.svg) [[!DNL Storefront popover]](storefront-popover.md)에는 가시성을 제어하는 `.search-autocomplete` 컨테이너에 대한 새 `active` 클래스가 있습니다.
![수정](../assets/fix.svg) 상점 앞에서 [검색어](https://experienceleague.adobe.com/ko/docs/commerce-admin/catalog/catalog/search/search-terms) 바닥글 링크가 제거되고 해당 캐시가 [!DNL Live Search] 설치에 사용할 수 없습니다.
검색 어댑터용 ![버그](../assets/bug.svg) 패치가 중복 제품을 처리합니다.
![버그](../assets/bug.svg) [!DNL Live Search]은(는) 다중(가상) [재고](https://experienceleague.adobe.com/ko/docs/commerce-admin/inventory/stocks/stocks-manage)가 있는 [단일 소스](https://experienceleague.adobe.com/ko/docs/commerce-admin/inventory/sources/sources-manage)&#x200B;(실제) 인벤토리 위치를 지원합니다. 지금은 여러 인벤토리 소스가 지원되지 않습니다.
...
![새로 만들기](../assets/new.svg) Commerce *관리자* 세션이 오랫동안 키보드를 사용하지 않는 동안에는 열려 있습니다
![신규](../assets/new.svg) [!DNL Live Search]은(는) 온보딩 후 자동으로 활성화됩니다
...
![버그](../assets/bug.svg) Facet을 추가할 때 `Update on Save`(으)로 설정하면 제품 특성 피드가 올바르게 업데이트되지 않습니다. 이 문제를 방지하려면 [색인 관리](https://experienceleague.adobe.com/ko/docs/commerce-admin/systems/tools/index-management)&#x200B;(으)로 이동하여 제품 특성 피드를 `Update by Schedule`(으)로 설정하십시오.
![버그](../assets/bug.svg) [!DNL Live Search] 동의어는 스토어 보기별로 정의되지만, 현재 웹 사이트별로 저장되고 `environmentId`과(와) `storeViewCode`의 조합으로 식별됩니다. 따라서 Adobe Commerce 설치 내의 모든 웹 사이트 및 스토어 보기는 동의어를 공유합니다. 저장소 보기에 대해 가장 최근에 생성된 동의어 집합이 우선합니다.
