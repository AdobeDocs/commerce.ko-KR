...
규칙에 대한 전략 유형을 선택합니다. **[!UICONTROL Test your rule]** 창에 예상 결과가 표시됩니다.

### 구성 가능한 제품 및 변형에 대한 동작 신호 {#behavioral-signals-variants}

**[!DNL Live Search]**&#x200B;은(는) 쇼핑객이 상호 작용하는 특정 제품에 대해 보기, 장바구니에 추가 이벤트 및 구매 등의 행동 신호를 수집합니다. 구성 가능한 제품의 경우, 이는 신호가 구성 가능한 상위 제품에 대해 기록되지 않고 **variant**(단순 제품) 수준에서 기록됨을 의미합니다.

구성 가능한 제품의 순위를 지정할 때 **[!DNL Live Search]**&#x200B;은(는) 모든 변형에서 수집된 동작 신호를 집계하여 구성 가능한 상위 항목으로 롤업합니다. 구성 가능한 제품의 등급 점수는 한 가지가 아닌 모든 변형의 결합된 신호를 반영합니다.
...
머천다이저는 카탈로그를 **200g** 및 **500g**&#x200B;과 같은 크기별 하위 범주로 구성합니다. 구성 가능한 제품에는 각 크기에 대해 하나씩, 두 개의 변형이 있습니다. 200g 변수만 200g 카테고리에 할당된 경우 500g 변형이 다른 곳에서 잘 판매되더라도 500g 변형의 구매 및 보기가 200g 카테고리 페이지에서 구성 가능한 제품의 순위 점수에 기여하지 않습니다. 그런 다음 구성 가능한 제품의 등급을 200g 카테고리 페이지에서 예상보다 낮게 매기거나 실제 판매 실적과 다르게 매길 수 있습니다. 두 변형을 각각의 카테고리에 할당하면 불일치가 해결됩니다.
...
이 컨트롤을 사용하면 검색 시 **텍스트 관련성**&#x200B;과 관련된 순서 지정에 **동작 신호**&#x200B;와 **범주 페이지** 및 **기본 목록**&#x200B;의 다른 순위 신호와 관련된 순서 지정에 영향을 주는 정도를 조정할 수 있습니다. **검색 쿼리 규칙**, **기본 규칙** 및 **카테고리 머천다이징 규칙**&#x200B;에 대해 증폭을 사용할 수 있습니다. 각 규칙은 고유한 값을 저장합니다.

| 비헤이비어 | 세부 사항 |
| --- | --- |
| 기본값 | `5`(이전의 고정된 동작 승수에 해당). |
| 범위 | `1`부터 `100`까지(더 부드러운 동작 영향). |
...
* **가장 많이 본 항목**&#x200B;과 같은 전략에서 모든 슬롯을 수동으로 고정하지 않고 모호하거나 광범위한 쿼리에 대해 더 적극적으로 참여도가 높은 SKU를 표시해야 하는 경우 **증폭을 올립니다**.
* 텍스트 일치 품질을 보다 엄격하게 관리하고 동작 데이터를 약간 이동하기만 하면 목록이 자동으로 조정됩니다. **내리기**
...
>[!NOTE]
>
> 같은 제품에서 높은 **[!UICONTROL Intelligent Ranking Boost]**&#x200B;이(가) **수동 증폭**&#x200B;보다 클 수 있습니다. 부스트된 SKU가 **[!UICONTROL Test your rule]** 또는 상점 첫 화면에서 예상한 것보다 낮은 순위를 갖는 경우 **[!UICONTROL Intelligent Ranking Boost]** 또는 **핀**&#x200B;을(를) 특정 위치로 낮춥니다. 두 변경 모두 결과에서 수동으로 순위가 매겨진 제품을 더 높게 이동합니다.

### 지능적인 순위 점수 책정 방식(검색)

**검색 규칙**(및 규칙 편집기의 테스트 쿼리)의 경우 지능형 등급은 두 가지 주요 요소인 **텍스트 관련성** 및 **동작 신호**&#x200B;를 결합하여 최종 제품 순서를 결정합니다. 이러한 요인이 상호 작용하는 방식을 이해하면 검색 결과에 대한 현실적인 기대를 설정하는 데 도움이 됩니다.
...
텍스트 관련성은 점수가 제한되지 않기 때문에 종종 순위를 지배하는 반면, 행동 영향은 부스트 모델에 의해 제한됩니다. 해당 규칙에 대해 **[!UICONTROL Intelligent Ranking Boost]**&#x200B;을(를) 늘리지 않는 한 텍스트 일치가 매우 강력한 제품은 참여도가 더 높은 SKU를 초과할 수 있습니다. 텍스트 일치 품질이 주요 드라이버로 남아 있으므로 더 높은 부스트 값에서도 텍스트 관련성 격차가 목록을 완전히 반전하지 않을 수 있습니다. 대상 쿼리에 대해 **[!UICONTROL Test your rule]**&#x200B;의 결과를 항상 확인하세요.
...
* **[!UICONTROL Boost]** - 검색 결과에서 SKU를 더 높게 이동합니다.
* **[!UICONTROL Bury]** - 검색 결과에서 SKU를 더 낮게 이동합니다.
* **[!UICONTROL Pin a product]** - 페이지에서 선택한 &quot;위치&quot;에 SKU가 표시됩니다.
* **[!UICONTROL Hide a product]** - 검색 결과에서 SKU를 제외합니다.
...
1. *이벤트*&#x200B;에서 연결된 조건이 충족될 때 수행할 **이벤트**&#x200B;을(를) 선택하십시오.
   ...
   예를 들어 **[!UICONTROL Hide a product]**&#x200B;을(를) 선택합니다. 그런 다음 숨길 제품의 이름을 입력합니다. 입력할 때 제품이 권장됩니다.
...
1. 완료되면 **저장 및 게시**&#x200B;를 클릭합니다.
   ...
   >[!IMPORTANT]
   >
   >**[!UICONTROL Save and publish]** 단추가 회색으로 표시된 경우 규칙 이름을 포함하여 규칙에 필요한 모든 정보를 입력했는지 확인하십시오.
...
| 이벤트 | 설명 |
|--- |--- |
| [!UICONTROL Boost] | 검색 결과에서 SKU 또는 SKU 범위를 더 높게 이동합니다. 각각은 테스트 검색 결과에 &quot;부스트&quot; 미리보기 배지로 표시됩니다. |
| [!UICONTROL Bury] | 검색 결과에서 더 낮은 SKU 또는 SKU 범위를 이동합니다. 각각은 테스트 검색 결과에 &quot;숨겨진&quot; 미리보기 배지로 표시됩니다. |
| [!UICONTROL Pin a product] | 단일 SKU를 검색 결과의 특정 위치에 연결합니다. 제품이 테스트 검색 결과에서 &quot;고정된&quot; 미리보기 배지로 표시됩니다. |
| [!UICONTROL Hide a product] | 검색 결과에서 SKU 또는 SKU 범위를 제외합니다. |
