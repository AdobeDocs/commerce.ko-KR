...
**지능형 순위**&#x200B;은(는) [Adobe AI](https://business.adobe.com/ai.html)의 상점 행동 데이터 분석을 활용하여 특정 알고리즘으로 선택한 범주 내의 모든 제품을 정렬합니다. 인텔리전트 등급을 선택하면 [!DNL Adobe AI]이(가) 기본 데이터를 지속적으로 재분석하므로 시간이 지남에 따라 특정 제품 순서가 변경됩니다. 예를 들어 상위 트렌드 제품은 쇼퍼 환경 설정이 변경될 때 시간이 지남에 따라 자동으로 변경됩니다.
지능형 순위 방법은 다음과 같습니다.
