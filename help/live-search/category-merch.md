...
**지능형 순위**&#x200B;은(는) [Adobe AI](https://business.adobe.com/kr/ai.html)의 상점 행동 데이터 분석을 활용하여 특정 알고리즘으로 선택한 범주 내의 모든 제품을 정렬합니다. 인텔리전트 등급을 선택하면 [!DNL Adobe AI]이(가) 기본 데이터를 지속적으로 재분석하므로 시간이 지남에 따라 특정 제품 순서가 변경됩니다. 예를 들어 상위 트렌드 제품은 쇼퍼 환경 설정이 변경될 때 시간이 지남에 따라 자동으로 변경됩니다.
지능형 순위 방법은 다음과 같습니다.

* 가장 많이 구매한 항목: 구매자가 이전 7일 동안 구매한 빈도별로 제품을 평가합니다.
* 장바구니에 가장 많이 추가: 이전 7일 동안 쇼핑객이 장바구니에 추가한 빈도별로 제품을 분류합니다.
* 가장 많이 본 항목: 이전 7일 동안 구매자가 제품을 본 빈도별로 제품을 분류합니다.
...
**None**&#x200B;을(를) 제외한 모든 지능형 순위 메서드에 대한 동작 신호가 제품 순서에 얼마나 강하게 영향을 주는지를 조정하려면 규칙 편집기에서 **[!UICONTROL Intelligent Ranking Boost]**&#x200B;을(를) 설정하십시오. 기본값, 제한, 미리 보기 동작 및 향상이 **수동 순위**&#x200B;와 비교되는 방법에 대한 자세한 내용은 [지능형 순위 향상](rules-add.md#intelligent-ranking-boost)을 참조하십시오.

**수동 순위**&#x200B;를 사용하면 수동 고정, 증폭, 저장 및 숨기기 규칙을 정의하여 자동 제품 정렬 순서를 재정의할 수 있습니다.

## 상속된 순위

머천다이저로서 &quot;트렌드&quot;별로 정렬할 모든 여성 의류 카테고리를 선택하십시오. 여기에는 하위 카테고리인 &quot;여성용 바지&quot;, &quot;여성용 셔츠&quot;, &quot;여성용 액세서리&quot;가 포함된다. 남성의 범주는 영향을 받지 않아야 합니다. 상속된 등급을 사용하여 이를 달성할 수 있습니다.
...
각 카테고리 또는 하위 카테고리는 한 번에 최대 1개의 지능형 순위가 적용될 수 있습니다. 하나 이상의 수동 순위가 동시에 적용될 수도 있습니다.

범주에 인텔리전트 등급을 적용하고 [!UICONTROL Apply intelligent ranking to subcategories]을(를) 활성화하면 해당 범주의 인텔리전트 등급이 하위 범주에 이미 적용된 모든 인텔리전트 등급을 대체합니다.
...
[!UICONTROL Apply intelligent ranking to subcategories]이(가) 활성화된 동안 범주에서 지능형 등급을 제거하면 해당 하위 범주에서 상속된 지능형 순위만 제거됩니다. 수동 순위는 상속되지 않으므로 유지됩니다.
...
1. 위에서 설명한 대로 범주에 대한 지능형 순위 규칙을 설정합니다.

   쿼리 결과가 [범주 미리 보기] 페이지 보기에 나타납니다. 이렇게 하면 실제 라이브 검색 데이터를 사용하여 결과를 미리 볼 수 있습니다.

1. 카테고리 페이지 미리보기 보기에서 제품을 클릭하여 드래그합니다. 원하는 위치에 드래그하여 놓습니다. 제품 및 위치 필드는 이벤트 창에서 자동으로 채워집니다.

고정 아이콘을 클릭하여 제품을 현재 위치로 잠글 수도 있습니다. 줄임표 컨텍스트 메뉴를 사용하여 &quot;맨 위에 고정&quot; 또는 &quot;맨 아래에 고정&quot;을 수행합니다.
