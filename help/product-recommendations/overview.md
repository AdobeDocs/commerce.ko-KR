...
[!DNL Product Recommendations] [Adobe AI](https://business.adobe.com/ai.html) 및 종합 구매자 행동 및 카탈로그 데이터에 대해 훈련된 머신 러닝을 사용하여 Adobe Commerce 상점 첫 화면에서 개인화된 추천을 제공합니다. 이 개요에서는 서비스 제한(HIPAA 포함), 데이터 개인정보 보호, 추천 단위 배치, 상점 구현 옵션, 제품 관계 통합 및 카탈로그 데이터 보존에 대해 설명합니다.
...
데이터 동기화에 대한 자세한 내용은 [데이터 관리 대시보드](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/data-transfer/data-sync/data-dashboard)를 참조하세요.

## 권장 사항 표시 위치

권장 사항은 &quot;이 제품을 본 고객이 본 항목&quot;과 같은 레이블이 있는 단위로 상점 전면에 표시됩니다. Adobe Commerce 관리에서 스토어 보기 간에 권장 사항을 만들고, 관리하고, 배포할 수 있습니다. Commerce 프로젝트에서 [Adobe Commerce Optimizer 커넥터](https://experienceleague.adobe.com/en/docs/commerce/aco-optimizer-connector/overview)를 사용하는 경우 [Adobe Commerce Optimizer](../optimizer/overview.md)을 통해 권장 사항을 만들고 관리하고 배포합니다.
...
- **EDS(Commerce Edge Delivery Services)** — [EDS에 대한 Adobe Commerce 상점 첫 화면 설명서](https://experienceleague.adobe.com/developer/commerce/storefront/get-started/)
...
[!DNL Product Recommendations] 서비스는 Adobe Commerce 환경과 계속 동기화되는 카탈로그 데이터에 따라 다릅니다. 해당 데이터의 쿼리를 중지하는 비활성 카탈로그 또는 환경은 비활성화될 수 있으며, 이는 다시 활성화하기 전까지 서비스가 반환하는 내용에 영향을 미칩니다.

연속 90일 동안 **테스트** 환경에서 카탈로그 데이터에 대한 쿼리를 제출하지 않으면 카탈로그 데이터가 비활성 모드로 설정되고 쿼리에 대해 데이터가 반환되지 않습니다. 90일 규칙은 **프로덕션** 환경의 카탈로그 데이터에 영향을 주지 않습니다.

생성 45일 후 환경에 **빈 카탈로그**&#x200B;가 있는 경우 카탈로그 데이터가 비활성 모드로 설정되고 쿼리에 대해 데이터가 반환되지 않습니다. 이는 프로덕션 및 테스트 환경 모두에 적용됩니다.

### 카탈로그 데이터 다시 활성화

카탈로그 데이터가 비활성화되면 복원하려면 &quot;[!DNL Product Recommendations] 다시 활성화&quot;라는 제목을 사용하여 [지원 요청을 제출](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#experience-league-start-page)하고 환경 ID를 포함하세요. 카탈로그 데이터는 2시간 이내에 복원해야 합니다.
