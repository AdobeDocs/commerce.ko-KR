...
### Fastly 이미지 최적화 지원 추가 {#fastlysupport}

[!DNL Product Recommendations]에 대한 Fastly 이미지 최적화 지원은 선택적 모듈이며 별도로 설치됩니다. 이 모듈은 [Fastly 이미지 최적화](https://experienceleague.adobe.com/en/docs/commerce-on-cloud/user-guide/cdn/fastly) 매개 변수를 [!DNL Product Recommendations] 이미지 URL에 적용합니다. 설치하려면 다음 명령을 실행합니다.
