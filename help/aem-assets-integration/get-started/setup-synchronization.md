...
AEM Assets 통합을 구성하기 전에 다음 단계를 완료했는지 확인하십시오.
...
Commerce에서 에셋 선택기를 사용하여 보다 매끄러운 설정을 제공하려면 다음 권한이 필요합니다.
...
IMS 인증은 기본적으로 활성화되어 있습니다. **AEM Assets**&#x200B;의 [Adobe Admin Console DM OpenAPI Users - delivery](https://adminconsole.adobe.com/) 제품 프로필에 사용자를 추가하여 AEM Assets 배달 계층에 대한 액세스 권한을 부여합니다.
...
1. [Commerce 관리 안내서](https://experienceleague.adobe.com/docs/commerce-admin/start/admin/ims/adobe-ims-config.html?lang=ko){target=_blank}의 지침에 따라 *Commerce용 Adobe IMS를 사용하도록 설정*&#x200B;합니다.
...
1. **AEM Assets**&#x200B;의 [Adobe Admin Console DM OpenAPI Users - delivery](https://adminconsole.adobe.com/) 제품 프로필에 사용자를 추가하여 AEM Assets 배달 계층에 대한 액세스 권한을 부여합니다.
...
>[!INFO]
>
> AEM Assets 통합은 글로벌(기본) 범위에서만 구성을 지원합니다. 웹 사이트 수준 구성은 지원되지 않습니다. 웹 사이트 수준에서 통합을 구성하려고 하면 시스템이 웹 사이트 수준 설정을 무시하고 대신 전역 구성 값을 사용합니다.

1. [!BADGE PaaS만]{type=Informative tooltip="Adobe Commerce on Cloud 프로젝트에만 적용됩니다(Adobe 관리 PaaS 인프라)."} **[!UICONTROL Asset Selector IMS Client ID]**&#x200B;을(를) 입력하십시오.

   이 ID는 프로그램 ID 및 환경 ID 필드에 대해 에셋 선택기 및 자동 채우기 기능을 활성화하는 데 필요합니다. 이 ID를 얻으려면 [IMS 및 사용자 권한](#ims-and-user-permissions)을(를) 참조하십시오. 자산 선택기에 대한 자세한 내용은 [자산 수동으로 선택](../synchronize/asset-selector-integration.md)을 참조하십시오.

1. 드롭다운 메뉴에서 AEM Assets 환경 **[!UICONTROL Program ID]** 및 **[!UICONTROL Environment ID]**&#x200B;을(를) 선택합니다.

   드롭다운은 사용자의 IMS 세션을 기반으로 자동으로 채워집니다. 이 기능을 사용하려면 올바른 [IMS 및 사용자 권한](#ims-and-user-permissions)이 있는지 확인하십시오.
...
* [Commerce 자산 관리](../manage-assets.md).
