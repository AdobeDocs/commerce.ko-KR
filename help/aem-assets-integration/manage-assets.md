...
제품 이미지는 일치 규칙을 통해 연결됩니다.

* AEM Assets에서 제품 자산(메타데이터, SKU 연결, 승인)을 추가하거나 업데이트하는 방법에 대해 알아보려면 [기본 자동 일치](synchronize/default-match.md)를 참조하십시오.
* 범주 이미지 또는 Page Builder 콘텐츠에 대해서는 [수동 자산 선택](synchronize/asset-selector-integration.md)을 참조하십시오.
...
1. **이미지 및 비디오** 섹션을 엽니다.
   ...
   제품 자산을 구성하려면(이미지를 SKU에 연결) AEM Assets 작성자 인스턴스를 열고 기본 보기에서 **Assets**&#x200B;을 클릭합니다. 메타데이터 구성 단계는 [기본 자동 일치](synchronize/default-match.md)를 참조하십시오.
...
AEM Assets의 제품(메타데이터 구성 및 승인 포함)에 에셋을 연결하는 방법은 다음 항목을 참조하십시오.
...
동기화 타이밍에 대한 자세한 내용은 [동기화 SLA](get-started/setup-synchronization.md#synchronization-sla) 항목을 참조하십시오.
...
[!BADGE Paa만 해당]{type=Informative tooltip="Adobe Commerce on Cloud 프로젝트에만 적용됩니다(Adobe 관리 PaaS 인프라)."} 이미지 임베드에 **AEM 자산 선택기**&#x200B;를 사용하려면 사용자에게 필요한 [권한 및 IMS 인증](get-started/permissions.md)이 있는지 확인하십시오.
...
[AEM 자산 선택기](synchronize/asset-selector-integration.md)를 구성하고 사용자에게 필요한 [권한 및 IMS 인증](get-started/permissions.md)이 있는지 확인한 후 이 옵션을 사용하여 자산을 카탈로그 범주 콘텐츠에 추가할 수 있습니다.
