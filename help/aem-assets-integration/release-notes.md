...
![새 문제](../assets/new.svg)<!-- Issue ACCS-169 --> **[!UICONTROL Program ID]** 및 **[!UICONTROL Environment ID]** 필드가 이제 사용자의 IMS 세션을 기반으로 드롭다운으로 자동 채워집니다. 이렇게 하면 수동으로 입력할 필요가 없어지고 구성 오류가 줄어듭니다. 이 기능을 사용하려면 사용자에게 [AEM 배달 계층](https://experienceleague.adobe.com/ko/docs/experience-manager-cloud-service/content/onboarding/journey/assign-profiles-aem#add-team-members){target=_blank}의 역할과 올바른 [자산 선택기 IMS 클라이언트 ID](https://experienceleague.adobe.com/ko/docs/commerce/aem-assets-integration/get-started/setup-synchronization#configure-the-connection){target=_blank}가 구성되어 있어야 합니다.
...
![문제 해결](../assets/fix.svg)<!-- Issue ACAP-1206 --> 자산 동기화 중에 자산 역할에 대한 사용자 지정 특성의 오래된 `no_selection` 값이 제거되지 않아 일부 이미지가 Edge Delivery Services에 올바르게 표시되지 않는 문제가 해결되었습니다.
...
![해결된 문제](../assets/fix.svg)<!-- Issue ACAP-1180 --> AEM 자산이 CDN에 의해 동적으로 최적화되었으므로 해당 자산의 파일 크기와 차원을 숨김으로써 제품 편집 페이지를 개선했습니다. 이제 AEM Assets 통합이 활성화되면 페이지가 올바르게 사전 렌더링됩니다.
