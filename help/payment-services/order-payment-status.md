...
1. 관리자의 `ON SAVE`인덱스 관리`BY SCHEDULE`에 구성된 방법에 따라 인덱스가 [(주문 정보 또는 저장소 정보가 변경될 때마다) 또는 ](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/tools/index-management)&#x200B;(사전 구성된 cron 일정에 따라)의 데이터를 변경합니다.
...
관리자`ON SAVE`에서 인덱스 모드—`BY SCHEDULE` 또는 [—](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/tools/index-management#change-the-index-mode)을(를) 변경할 수 있습니다.
