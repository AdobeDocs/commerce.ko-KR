...
[!DNL Payment Services] 및 [!DNL Adobe Commerce]에 대한 [!DNL Magento Open Source]은(는) 스토어의 [거래](reporting.md), 주문 및 결제를 명확하게 볼 수 있도록 포괄적인 보고를 제공합니다.
...
자세한 표 형식 **[!UICONTROL View Report]**&#x200B;주문 결제 상태 보고서 보기[(으)로 이동하려면 &#x200B;](#order-payment-status-report-view)을(를) 클릭하십시오.
...
1. 관리자의 `ON SAVE`인덱스 관리`BY SCHEDULE`에 구성된 방법에 따라 인덱스가 [(주문 정보 또는 저장소 정보가 변경될 때마다) 또는 &#x200B;](https://experienceleague.adobe.com/ko/docs/commerce-admin/systems/tools/index-management)&#x200B;(사전 구성된 cron 일정에 따라)의 데이터를 변경합니다.
...
>[!NOTE]
>
>이 표에 표시된 데이터는 기본적으로 `DESC`을(를) 사용하여 내림차순(`ORDER DATE`)으로 정렬됩니다. `ORDER DATE`은(는) 주문이 만들어진 날짜 타임스탬프입니다.
...
관리자`ON SAVE`에서 인덱스 모드—`BY SCHEDULE` 또는 [—](https://experienceleague.adobe.com/ko/docs/commerce-admin/systems/tools/index-management#change-the-index-mode)을(를) 변경할 수 있습니다.
...
1. 특정 주문에 대한 모든 분쟁을 확인하고 [분쟁 상태](#statuses-information)를 확인하세요.
1. [PP-D-](https://www.paypal.com/us/cshelp/article/what-is-the-resolution-center-help246)&#x200B;(으)로 시작하는 분쟁 ID 링크를 클릭하여 _PayPal 해결 센터_&#x200B;에서 분쟁 세부 정보를 검토하십시오.
...
1. _관리자_ 사이드바에서 **[!UICONTROL Sales]** > **[!UICONTROL [!DNL Payment Services]]** > _[!UICONTROL Orders]_>**[!UICONTROL View Report]**(으)로 이동합니다.
1. 지난 30일 이외의 다른 기간에 대한 상태를 보려면 [해당 상태에 대한 날짜 범위 기간을 사용자 지정](#customize-order-dates-timeframe)하세요.
