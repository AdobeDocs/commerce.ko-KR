...
이 릴리스 노트는 [!DNL Payment Services]의 최신 버전에 대해 설명합니다.

Adobe은 릴리스된 최신 결제 서비스 버전에 대한 지원을 제공합니다. 이전 버전에 대한 릴리스 노트는 참조를 위해 제공됩니다.
...
![새 문제](../assets/new.svg)<!-- Issue PAY-6510 --> [!DNL Adobe Commerce as a Cloud Service]의 경우 [!DNL Commerce Admin]에 있는 [!DNL Payment Services] 대시보드 UI를 통해 가맹점이 대시보드에서 직접 [거래 보고서](reporting.md#transactions-report-view)를 열 수 있습니다. 이 SaaS 대시보드는 **선택한**&#x200B;개의 보고만 노출합니다(클라우드와 온-프레미스에서 Adobe Commerce에 사용할 수 있는 모든 보고서 또는 홈 진입점은 아님). [[!DNL Payment Services] 홈](payments-home.md)을 참조하세요. 릴리스 노트 및 시기를 [!DNL Adobe Commerce as a Cloud Service]하려면 [결제 서비스 거래 보고서 보기](../cloud-service/release-notes.md#view-the-payment-services-transactions-report)를 참조하십시오.

_2026년 1월 21일_

![새 문제](../assets/new.svg)<!-- Issue PAY-6374 --> 이제 [!DNL Payment Services] 대시보드의 **설정** 단추가 결제 방법에 대한 관리자 구성 페이지로 리디렉션되어 결제 설정 관리를 위한 보다 간소화된 워크플로우를 제공합니다.

_2026년 1월 19일_

![새로운 문제](../assets/new.svg)<!-- Issue PAY-6325 --> 이제 판매자는 PayPal 판매자 ID를 [거래 보고서](reporting.md#transactions-report-view)의 열로 보고 특정 고객이 수행한 거래를 식별할 수 있습니다.
...
![새 문제](../assets/new.svg)<!-- Issue PAY-5658 --> 이제 가맹점은 보다 자세하고 정확한 결제 방법 데이터를 위해 [거래 보고서](reporting.md#transactions-report-view)의 결제 세부 정보별로 거래를 필터링할 수 있습니다.

_2024년 7월 15일_

![새 문제](../assets/new.svg)<!-- Issue PAY-5571 --> 이제 판매자는 [거래 보고서](reporting.md#transactions-report-view)에서 Commerce 고객 전자 메일로 거래를 필터링할 수 있습니다. 해당 특정 이메일에 대한 트랜잭션을 필터링할 고객 이메일을 입력합니다.

_2024년 7월 9일_

![새 문제](../assets/new.svg)<!-- Issue PAY-5488 --> 이제 판매자는 [거래 보고서](reporting.md#transactions-report-view)에서 Commerce 고객 ID를 열로 보고 특정 고객이 수행한 거래를 식별할 수 있습니다. 또한 가맹점은 연결된 주문에 대해 이 Commerce 고객 ID로 트랜잭션 보고서를 필터링할 수 있습니다.
...
![새 문제](../assets/fix.svg)<!-- Issue PAY-4888 --> 이제 가맹점은 [거래 보고서](reporting.md#transactions-report-view)에서 신용 카드 및 직불 카드 거래를 카드 번호의 마지막 4자리까지 필터링할 수 있습니다.
...
![새로 만들기](../assets/new.svg)<!-- Issue PAY-4288 --> 이제 판매자는 [PayPal 결제 단추를 _만_&#x200B;구성](payments-options.md#use-only-paypal-payment-buttons)할 수 있으며 _사용할 수 없음_&#x200B;은 PayPal 신용카드 결제 옵션을 사용합니다. 이를 통해 가맹점은 벤모와 페이팔 결제 버튼 등 다양한 결제 옵션을 제공하고, 페이팔 신용카드 결제 옵션 대신 기존 신용카드 제공업체를 이용할 수 있다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-4050 --> 주문 결제 상태 보고서에 대해 결제 서비스 홈에 표시되는 [데이터 시각화 보기](payouts.md#payouts-data-visualization-view)를 추가했습니다.

![해결된 문제](../assets/fix.svg)<!-- Issue PAY-4486--> 이전에는 PayPal PayLater 단추가 영국 판매자의 체크아웃에 표시되지 않았습니다. 해당 문제는 해결되었습니다.

[!DNL Payment Services]이(가) 비활성화되면 ![해결된 문제](../assets/fix.svg)<!-- Issue PAY-4485--> 보고서 데이터 시각화 보기가 이제 [!DNL Payment Services] 홈에 표시됩니다.
...
![새로 만들기](../assets/new.svg)<!-- Issue PAY-3705 --> 이제 `increment_id`을(를) 외부 ERP 시스템에서 급여 조정 시 사용할 수 있습니다. `custom_id` _및_ `invoice_id`(으)로 전파되었으며, PayPal 웹후크 및 지불에 대한 판매자 활동 세부 정보에 모두 표시됩니다.
...
![새로 만들기](../assets/new.svg)<!-- Issue PAY-3420 --> 이제 Apple Pay를 PayPal 스마트 단추로 사용할 수 있습니다. 이 [결제 옵션](payments-options.md#apple-pay-button)을 통해 고객은 iOS 또는 macOS 장치에서 Touch ID 기능을 사용하여 Apple Pay를 선택할 수 있습니다. Apple Pay는 디바이스에 저장된 신용 및 직불 카드 결제 자격 증명을 사용하여 결제를 처리합니다.

_2021년 6월 28일_

![새로 만들기](../assets/new.svg)<!-- Issue PAY-1720 --> 스토어 주문에 대한 분쟁은 이제 [주문 결제 상태 보고서](order-payment-status.md#view-disputes)에서 사용할 수 있습니다. [!DNL Payment Services]에서 PayPal 해결 센터로 직접 이동하여 문제를 해결할 수 있습니다.

[!DNL Payment Services] 홈의 ![새로 만들기](../assets/new.svg)<!-- Issue PAY-2854 --> 사용자 경험에는 현재 상속 수준에서 구성을 수정하는 기능과 헤더 및 탐색 표시에 대한 개선 사항이 포함되어 있습니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-2854 --> 이제 샌드박스 모드에서 프로덕션 모드로 전환하거나 저장되지 않은 업데이트가 있는 보기에서 나가려고 하면 경고가 표시됩니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-2761 --> 이제 열 설정 컨트롤을 사용하여 열을 표시하거나 숨김으로써 [주문 결제 상태 보고서](order-payment-status.md#show-and-hide-columns) 및 [결제 보고서](payouts.md#show-and-hide-columns)에 표시되는 데이터를 사용자 지정할 수 있습니다.
...
![새로 만들기](../assets/new.svg)<!-- PAY-6564 --> 이제 [!DNL Google Pay] 빠른 체크아웃에서 클라이언트측 배송 콜백을 사용하여 Google Pay Sheet에 배송 방법을 표시할 수 있으므로 구매자는 결제를 확인하기 전에 배송을 선택합니다. 가맹점은 [Google 페이 구성](configure-admin.md#google-pay)에서 **[!UICONTROL Skip Review]**&#x200B;을(를) 사용하여 별도의 Google 페이 검토 단계를 표시하거나 숨길 수 있습니다. 검토 단계를 생략하면 전자 지갑 주소 또는 운송 방법이 변경될 때 운송 옵션이 시트에 남아 있습니다.

![신규](../assets/new.svg)<!-- PAY-6565 --> 이제 구매자는 Google 결제 검토 단계가 나타나는지 여부에 관계없이 [!DNL Google Pay] 신속 지급 시트에서 프로모션 코드를 적용할 수 있습니다. 주문당 여러 개의 쿠폰이 가능한 매장에서도 페이시트당 하나의 코드만 적용 가능하다. 자세한 내용은 [결제 옵션](payments-options.md#google-pay-button)을 참조하세요.

![새로 만들기](../assets/new.svg)<!-- PAY-6567 --> 이제 Safari와 함께 Chrome, Firefox, Microsoft Edge에서 [!DNL Apple Pay] 빠른 체크아웃을 사용할 수 있습니다. 데스크탑에서 쇼핑객은 iOS 18 이상을 실행하는 iPhone으로 QR 코드를 스캔하여 Apple Pay Sheet에서 결제를 완료할 수 있습니다. 빠른 배치(예: 제품 세부 정보 페이지에서)는 [Apple Pay 구성](configure-admin.md#apple-pay)에서 활성화해야 합니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6566 --> 이제 구매자는 [!DNL Apple Pay] 신속 지급 시트에서 단일 할인 코드를 적용하거나 제거할 수 있으며, 시트에서 합계가 업데이트됩니다. 평소대로 관리자에서 장바구니 가격 규칙을 만들고 활성화합니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6151 --> 이제 Luma 제품 세부 사항 및 체크아웃 페이지의 [!DNL Apple Pay]이(가) 결제 SDK을 통해 렌더링되므로 경험이 빠른 체크아웃에 사용되는 최신 PayPal 통합과 일치합니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6611 --> 이제 [!DNL Adobe Commerce] 주문 세부 사항 페이지에 [!DNL PayPal] 거래 ID, PayPal 디버그 ID, 지불인 이메일, 판매자 보호 자격, AVS/CVV 인증 코드 등 [!DNL Payment Services]을(를) 통해 처리된 거래에 대한 추가 결제 세부 사항이 표시됩니다. 자세한 내용은 [3DS](security.md#3ds)을(를) 참조하십시오.

![문제 해결](../assets/fix.svg)<!-- PAY-6513 --> 결제 시도 실패 및 나중에 성공적으로 체크아웃한 후에도 주문 성공 페이지에 [!DNL Apple Pay] 오류 메시지가 계속 표시되는 문제를 해결했습니다.

![문제 해결](../assets/fix.svg)<!-- PAY-6585 --> 주/지역이 필수 필드가 아닌 영국 및 유럽 주소에 대해 [!DNL Apple Pay] 신속 지급표에 **[!UICONTROL Please select a state/region]** 오류가 표시되는 문제가 해결되었습니다.
...
![새로 만들기](../assets/new.svg)<!-- PAY-6400 --> PayPal에 대한 전송 줄 항목 및 금액 분류를 사용하거나 사용하지 않도록 설정하는 **[!UICONTROL Line Items Enabled]** 구성을 추가했습니다. 이 설정은 타사 확장에서 [!DNL Payment Services]에서 지원하지 않는 사용자 지정 요금을 추가할 때 발생하는 체크아웃 문제를 해결하는 데 도움이 됩니다. 자세한 내용은 [라인 항목](line-items.md)을 참조하세요.

![새로 만들기](../assets/new.svg)<!-- PAY-6457 --> 체크아웃 시작 시 [빠른 결제 버튼이 추가됨](payments-options.md#express-checkout-buttons) 더 빠른 체크아웃이 장려됩니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6458 --> 지역별 및 로컬 결제 옵션에 대한 [로컬 결제 방법(LPM)](payments-options.md#local-payment-methods)에 대한 지원을 추가했습니다. 가맹점은 [Commerce](configure-admin.md#local-payment-methods) 내에서 사용 가능한 LPM을 직접 활성화하거나 비활성화할 수 있습니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6377 --> 구매자의 위치를 시뮬레이션하고 렌더링되는 결제 방법을 제어하기 위해 [구매자의 국가](sandbox.md#buyers-country) 샌드박스 구성을 추가했습니다.

![해결된 문제](../assets/fix.svg)<!-- PAY-6458 --> 구매자 전자 메일 주소 데이터를 전달하여 사기 방지 기능을 향상시켜 PayPal의 사기 필터 호환성을 개선했습니다.

![문제 해결](../assets/fix.svg)<!-- PAY-6413 --> 배송 주소가 누락되어 가상 제품에 대해 PayPal 체크아웃을 완료할 수 없는 문제를 해결했습니다.

![문제 해결](../assets/fix.svg)<!-- PAY-6450 --> 제품 세부 사항 페이지, 미니카트, 장바구니 및 맨 위의 체크아웃 시 PayPal 단추 체크아웃 오류가 올바르게 처리되지 않는 문제를 해결했습니다.

![문제를 해결했습니다](../assets/fix.svg)<!-- PAY-6453 --> 체크아웃 시 충돌을 방지하기 위해 GraphQL 체크아웃 시 서버측 전달 콜백 흐름을 사용하지 않도록 설정했습니다.

![해결된 문제](../assets/fix.svg)<!-- PAY-6456 --> Apple에서 확인되지 않은 도메인의 결제를 거부함을 명확히 하기 위해 [Apple Pay](configure-admin.md#apple-pay) 구성 설명을 업데이트했습니다. 가맹점은 Apple 결제 버튼이 사용되는 모든 도메인을 확인해야 합니다. 국가 제한이 적용됩니다.

## v2.13.3

_2026년 1월 14일_

![해결된 문제](../assets/fix.svg)<!-- PAY-6399 --> 관리자 체크아웃 중에 잘못된 저장된 카드가 사용되던 문제를 해결했습니다. 이제 선택한 카드가 주문 시 올바르게 사용됩니다.

## v2.13.2

_2026년 1월 5일_

![문제 해결](../assets/fix.svg)<!-- PAY-6390 --> 향상된 JS 축소를 구현하여 일반적인 개선 사항 및 최적화를 구현했습니다.

## v2.13.1

_2025년 12월 18일_

![해결된 문제](../assets/fix.svg)<!-- PAY-6355 --> 체크아웃하는 동안 판매자가 배송 방법을 선택할 수 없는 문제를 해결했습니다.

![문제 해결](../assets/fix.svg)<!-- PAY-6347 --> 불필요한 API 호출을 제거하여 체크아웃 복원 기능을 개선했습니다.

![문제를 해결했습니다](../assets/fix.svg)<!-- PAY-6368 --> 기본 도메인을 사용할 수 없을 때 결제 SDK을 로드하기 위한 대체 메커니즘을 구현하여 체크아웃 중에 신용 카드 필드에 계속 액세스할 수 있도록 했습니다.
...
![문제 해결](../assets/fix.svg)<!-- PAY-6164 --> 이제 [!DNL Payment Services]은(는) [PayPal SSSC(서버측 배달 콜백)](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/)에서 사용 가능한 배달 방법에 기본 통화를 사용합니다.
...
![새로 만들기](../assets/new.svg)<!-- PAY-6168 -->에서 [!DNL Payment Services]에 [`addProductsToNewCart`](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/mutations/) 돌연변이를 추가하여 더 원활한 전환과 더 나은 장바구니 재사용을 가능하게 했습니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6169 --> 견적 수명 주기 관리를 개선하기 위해 [!DNL Payment Services]에 [`setCartAsInactive`](https://developer.adobe.com/commerce/webapi/graphql/payment-services-extension/mutations/) 돌연변이를 추가했습니다.

![새로 만들기](../assets/new.svg)<!-- PAY-6227 --> PayPal을 사용하여 체크아웃할 때 [!DNL Payment Services]에서 더 빠른 구매 프로세스를 위해 주문 확인 팝업을 건너뜁니다. PayPal 검토 페이지에서 직접 배송 및 합계를 계산하는 [서버측 배달 콜백](payments-options.md#server-side-shipping-callbacks-for-paypal-payment-buttons)을 통해 사용할 수 있습니다.
...
![새로 만들기](../assets/new.svg)<!-- PAY-5938 --> 이제 [!DNL Payment Services]을(를) 통해 가맹점은 결제 설정을 관리하여 비즈니스의 유연성을 극대화할 수 있습니다. 이 버전은 판매자가 지원하는 지역 및 브랜드에 대해 [여러 PayPal 계정](configure-admin.md#use-multiple-paypal-accounts)을 첨부하는 기능을 향상시킵니다. 우리 영업팀은 웹 사이트 및 스토어 보기 범위를 설정하기 위한 온보딩 링크를 제공할 수 있습니다.
...
![새로 만들기](../assets/fix.svg)<!-- PAY-5789 --> [!DNL Payment Services]은(는) 이제 Google Pay를 사용한 [3D 보안 인증](security.md#3ds)을 지원하므로 결제 거래 중 가맹점과 고객에 대한 보안이 강화됩니다.

![수정](../assets/fix.svg)<!-- PAY-5703 --> [!DNL Payment Services]은(는) [고객이 자신의 **내 계정**](vaulting.md#vaulting-without-purchase)&#x200B;에서 직접 카드를 저장할 수 있는 기능을 추가하여 편리성을 높이고 향후 체크아웃을 간소화합니다. `Vault without purchase functionality might not be 100% compatible with Adobe Commerce 2.4.4 due to a known issue with` [`GraphQL authorization mechanisms`](https://developer.adobe.com/commerce/webapi/graphql/usage/authorization-tokens/).

![수정](../assets/fix.svg)<!-- PAY-5762 --> 주문이 PDP(제품 세부 사항 페이지)에서 시작된 경우 주문 검토 페이지에 쿠폰 코드가 적용되지 않는 문제를 해결했습니다.

![수정](../assets/fix.svg)<!-- PAY-5792 --> [!DNL Payment Services]은(는) 이제 체크아웃 페이지](vaulting.md)에 [저장된 카드에 대한 설명과 청구 주소를 표시하여 고객이 저장된 결제 방법을 보다 잘 알 수 있도록 합니다.
...
![새로 만들기](../assets/new.svg)<!-- PAY-5380 --> [!DNL Payment Services]은(는) 더 쉽고 직관적인 온보딩 프로세스를 위해 판매자를 위한 [관리](configure-admin.md#general-configuration) 환경의 구성을 개선합니다. 이 기능을 사용하면 판매자가 [!DNL Payment Services] ID를 재설정할 수 있습니다.
...
![수정](../assets/fix.svg)<!-- PAY-5492 --> 판매자가 사용자 지정된 약관을 체크아웃 페이지에 추가할 때 발생하는 문제를 해결했습니다. [빠른 체크아웃](payments-options.md#standard-vs-advanced-payments-experience)을 진행하는 동안 쇼핑객은 이제 문제 없이 체크아웃을 완료할 수 있도록 이 사용 약관에 동의할 수 있습니다.
...
![신규](../assets/new.svg)<!-- PAY-4877 --> [!DNL Payment Services]에서 [L2/L3 가격 기능을 지원합니다](levels-card-payment-transactions.md#level-2-and-level-3). 이 기능은 IC++ 가격이 활성화된 [!DNL Payment Services] 고객에게만 제공됩니다. [!DNL Payment Services]에 대한 L2/L3 처리 데이터를 사용하려면 [!DNL Payment Services] 계정 관리자에게 문의하십시오.
...
![수정](../assets/fix.svg)<!-- PAY-5291 --> 일부 거래에 대한 결제 확인 수신이 지연될 수 있습니다. 이 경우 이제 가맹점은 주문에 대해 업데이트된 결제 상태를 받을 수 있습니다. [결제 서비스는 보류 중인 거래를 감지하고 이러한 거래를 사전에 모니터링하여 보류 중인 상태가 캡처되면 업데이트하여 순서대로 결제 거래의 보류 상태를 감지합니다](order-payment-status.md#payment-status-updates).
...
- 상점 첫 화면의 고객 주문 페이지
- 구매자에게 전송된 주문 확인 이메일
- Commerce 관리자의 [주문 세부 사항 보기](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/order-management/orders/order-processing#view-an-order)에서.
...
![새로 만들기](../assets/new.svg)<!-- PAY-4638 --> 자동화된 사기 방지 서비스를 제공하는 Signifyd](fraud-protection.md)와의 [통합을 추가했습니다.

![새로 만들기](../assets/new.svg)<!-- PAY-3981 --> [Apple Pay를 PayPal 결제 단추 외부에 있는 별도의 결제 옵션으로 승격](payments-options.md#apple-pay-button)하여 쇼핑객이 결제 옵션을 볼 수 있도록 하고 가맹점이 Apple Pay의 배치와 스타일을 제어할 수 있도록 합니다.

![새로 만들기](../assets/new.svg)<!-- PAY-4002 --> 결제 아이콘 추가와 같은 스타일 개선 사항을 포함하여 신용카드 필드 체크아웃의 사용자 환경을 개선하여 쇼핑객 인지 부하를 줄이고 전환을 증가시켰습니다.

![새로 만들기](../assets/new.svg)<!-- PAY-4002 --> 가맹점이 특정 결제 옵션의 우선 순위를 지정하기 위해 [결제 옵션의 순서를 정렬](configure-admin.md#paypal-payment-buttons)할 수 있는 기능을 추가했습니다. 이 기능을 사용하면 체크아웃 대화율이 높아집니다.

![새로 만들기](../assets/new.svg)<!-- PAY-4035 --> 판매자는 이제 [!DNL Payment Services] 관리 홈 페이지에서 사용할 수 있는 새 [트랜잭션 보고서](reporting.md#transactions-report-view)를 사용하여 저장소 상태를 효율적으로 모니터링하고 트랜잭션 문제를 식별할 수 있습니다. 보고서는 거래 승인률과 부정적 거래 추세에 대한 자료도 제시한다.
...
![새로 만들기](../assets/new.svg)<!-- Issue xxx --> 다음 국가 및 관련 통화에서 [사용 가능 여부](compatibility.md#standard-vs-advanced-payment-services-experience) 추가: 호주, 프랑스, 영국

![새로 만들기](../assets/new.svg)<!-- Issue PAY-4296 --> 관리자 사용자가 고객에 대한 주문을 만들고 관리할 수 있도록 [관리자 역할에 대한 확장된 리소스](configure-admin.md#configure-roles)를 추가하고 [!DNL Payment Services]을(를) 판매 메뉴에서 볼 수 있습니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-4236 --> 체크아웃 중에 오류가 발생하는 주문에 대해 [자동 무효화를 추가했습니다](checkout.md#order-auto-voided-if-error).

![새로 만들기](../assets/new.svg)<!-- Issue PAY-4183 --> 체크아웃 페이지에서 [신용/직불 카드 결제 옵션 단추 표시](payments-options.md#paypal-debit-or-credit-card-button)하는 기능을 만들었습니다.
...
![수정](../assets/fix.svg)<!-- Issue PAY-4226 --> 새 [!DNL Payment Services] 판매자가 관리자의 체크아웃을 사용하지 못하는 문제를 해결했습니다.[!DNL Payment Services] 는 이전에 새 고객에 존재하지 않는 Commerce 고객 ID를 사용했습니다.

![수정](../assets/fix.svg)<!-- Issue PAY-4205 --> [PayPal 옵션](payments-options.md#paypal-payment-buttons)을 사용하여 체크아웃하는 동안 지정된 배송 주소 상태가 기본 세금 설정의 상태로 바뀌는 문제를 해결했습니다. 이제 고객은 머천트의 세금 설정에서 기본값으로 구성된 상태 이외의 상태로 주문을 출하할 수 있습니다.

![수정](../assets/fix.svg)<!-- Issue PAY-4202 --> 고객이 카드 보관을 사용하여 구매를 완료하거나 저장소 [결제 작업 `Authorize and Capture`을(를) 사용하여 보관된 결제 방법을 삭제할 수 없는 문제를 해결했습니다](production.md#set-payment-services-as-payment-method). 이전에는 고객이 저장된 신용 카드를 사용하거나 수정하려고 할 때 &quot;Provider Vault ID를 찾을 수 없음&quot; 오류가 표시되었습니다.
...
![신규](../assets/new.svg)<!-- Issue PAY-3950 --> 상인이 이제 상점에 대해 [즉시 구매 Commerce 기능](https://experienceleague.adobe.com/en/docs/commerce-admin/stores-sales/point-of-purchase/checkout-instant-purchase)을 사용하도록 설정하여 쇼핑객이 체크아웃을 신속하게 할 수 있도록 할 수 있습니다([저장된 신용 카드 정보](vaulting.md) 사용).
...
![신규](../assets/new.svg)<!-- Issue PAY-784 -->[!DNL Payment Services]에는 [여러 PayPal 비즈니스 계정을 사용](configure-admin.md#use-multiple-paypal-accounts)할 수 있도록 판매자 계정을 설정할 수 있는 기능이 포함됩니다. 이를 통해 판매자는 다양한 통화를 사용하여 여러 국가에서 스토어를 운영하거나 비즈니스의 일부로 Adobe Commerce을 사용할 수 있습니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-3231 --> 판매자는 [고객 거래 은행 거래 내역에 표시되는 웹 사이트 또는 개별 스토어 보기 구성에 [!UICONTROL Soft Descriptor]](configure-admin.md)을(를) 추가하여 브랜드, 스토어 또는 제품 라인을 구분할 수 있습니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-3707 --> [!DNL Payment Services] 설정에서 [신용 카드 필드와 PayPal 결제 단추를 사용하거나 사용하지 않도록 설정](configure-admin.md#paypal-payment-buttons)합니다.
...
![알려진 문제](../assets/bug.svg)<!-- Issue PAY-0 --> [!DNL Payment Services]이(가) `commerce-data-export` v101.20 이상과 호환되지 않으므로 [[!DNL Channel manager] 확장](https://experienceleague.adobe.com/en/docs/discontinued/using/commerce-channels)&#x200B;(중단됨)과 호환되지 않습니다.
...
![새로 만들기](../assets/new.svg)<!-- Issue PAY-2682 --> 이제 캐나다 상인이 [!DNL Adobe Commerce] 및 [!DNL Magento Open Source]에 대한 [!DNL Payment Services] 확장을 사용할 수 있습니다. 가맹점은 [프랑스어](compatibility.md##standard-vs-advanced-payment-services-experience) 또는 [영어](compatibility.md#standard-vs-advanced-payment-services-experience)로 결제 구성을 볼 수 있습니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-2681 --> [!DNL Payment Services]은(는) 신용 카드 및 PayPal 거래에 대해 [캐나다 달러(CAD)](compatibility.md#standard-vs-advanced-payment-services-experience)를 지원합니다.
...
![해결된 문제](../assets/fix.svg)<!-- Issue PAY-2277 --> 이제 선택적으로 [개별 PayPal 결제 단추를 사용하거나 사용하지 않도록 설정](configure-admin.md#paypal-payment-buttons)할 수 있습니다.

![해결된 문제](../assets/fix.svg)<!-- Issue PAY-2561 --> 이전에 제거한 제품이 _주문 검토_ 페이지의 장바구니에 표시되지 않습니다.

![알려진 문제](../assets/bug.svg)<!-- Issue PAY-2842 --> 샌드박스 환경에서 결제를 처리할 때 신용 카드 거래 테스트 [PayPal로 실패](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26825)할 수 있습니다.
...
![새로 만들기](../assets/new.svg)<!-- Issue PAY-124 --> [!DNL Adobe Commerce] 및 [!DNL Magento Open Source]에 대한 [!DNL Payment Services] 확장은 [[!DNL Adobe Commerce] 클라우드 인프라](install.md#adobe-commerce-on-cloud-infrastructure) 또는 [온-프레미스](install.md#on-premises-and-other-configurations) 인스턴스에 대해 설치할 수 있습니다. 이러한 메서드는 명령줄 인터페이스를 사용해야 합니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-1986 --> [!DNL Payment Services]은(는) 판매자가 테스트 모드에서 확장을 평가할 수 있도록 하는 [샌드박스 계정](sandbox.md)을 지원합니다.

![새로 만들기](../assets/new.svg)<!-- Issue PAY-666 --> 가맹점은 샌드박스 또는 프로덕션 환경 간 [`Authorize and Capture`](production.md#set-payment-services-as-payment-method) 전환을 사용하는 것과 같은 기본 결제 동작을 사용하여 [결제 서비스를 구성](configure-admin.md) 확장을 구성할 수 있습니다.
...
![알려진 문제](../assets/bug.svg)<!-- Issue PAY-2473 --> 확장을 설치하는 동안 [잘못된 작성기 키](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26826)를 사용하면 사용자가 올바른 `MAGEID`을(를) 사용하여 [인증](https://experienceleague.adobe.com/en/docs/commerce-operations/installation-guide/prerequisites/authentication-keys)할 수 없습니다.

![알려진 문제](../assets/bug.svg)<!-- Issue PAY-2474 --> [!DNL Payment Services]개의 보고서 [을(를) 즉시 동기화할 수 없습니다](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26837).

![알려진 문제](../assets/bug.svg)<!-- Issue PAY-2475 --> 온보딩 중에 해당 계정을 만든 경우 [!DNL Payment Services]에 대한 [PayPal 샌드박스 계정](https://experienceleague.adobe.com/en/docs/experience-cloud-kcs/kbarticles/ka-26836)을(를) 확인할 수 없습니다.
