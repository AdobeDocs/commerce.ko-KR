...
관리자의 [!DNL Adobe Commerce] 및 [!DNL Magento Open Source]에 대해 [!DNL Payment Services]을(를) 구성할 때 해당 구성은 _[!UICONTROL General Configuration]_의_[!UICONTROL Method]_ 필드에 설정된 환경에만 적용됩니다. 구성 필드에서 변경한 내용은 _[!UICONTROL Method]_선택 항목을 전환하는 것과 관련이 없습니다. 메서드를 전환하는 경우 선택 항목이 재설정되지 않습니다.
...
스토어와 _[!UICONTROL Merchant Location]_에 대해 [!DNL Payment Services]을(를) 사용하도록 설정하고_[!UICONTROL General Configuration]_ 섹션에서 샌드박스 테스트 또는 실시간 결제를 사용하도록 설정할 수 있습니다.
...
1. [Commerce 서비스 커넥터](../landing/saas.md){target=_blank}를 설정하고 [!DNL Payment Services] 대시보드를 처음 방문하면 **[!UICONTROL Payment Services Sandbox ID]** 및 **[!UICONTROL Payment Services Production ID]** 값이 자동으로 채워집니다. 이렇게 하여 샌드박스 및/또는 프로덕션 환경에 대한 온보딩을 완료합니다. 이 값은 SaaS ID를 [!DNL Payment Services]에 연결합니다.
...
이전 릴리스에서는 웹 사이트 수준의 PayPal 계정 매핑을 통해 일반적으로 [지원 팀에 문의](https://experienceleague.adobe.com/en/docs/support-resources/adobe-support-tools-guide/adobe-commerce-support/adobe-commerce-help-center-user-guide#support-case) 또는 Adobe 담당자에게 문의해야 했습니다. 아래 필수 구성 요소를 충족하면 **[!UICONTROL Connect different account]** 작업을 사용하십시오.
...
1. [Commerce 서비스 커넥터](../landing/saas.md) 설정이 완료되었습니다.

1. [샌드박스 및 프로덕션](connect.md#configure-commerce-services) API 키(공개 및 개인)가 관리자에 저장됩니다.

1. **[!UICONTROL Payment Services Sandbox ID]** 및 **[!UICONTROL Payment Services Production ID]**&#x200B;이(가) [일반 구성](#general-configuration)에서 채워집니다.
...
자세한 내용은 [결제 옵션](payments-options.md#paypal-payment-buttons)을 참조하세요.
...
| [!UICONTROL Payment Action] | 웹 사이트 | 지정한 결제 방법에 대한 [결제 작업](https://experienceleague.adobe.com/en/docs/commerce-admin/config/sales/payment-methods/payment-methods). 옵션: [!UICONTROL Authorize] / [!UICONTROL Authorize and Capture] |
...
1. 활성화하려면 **[!UICONTROL Enable Fastlane]**&#x200B;에 대해 `Yes`을(를) 선택합니다(`No`이(가) 비활성화함).
...
1. 체크아웃 페이지에서 결제 방법의 우선 순위를 지정하려면 **[!UICONTROL Sort order]** 필드에 `Numeric Only` 값을 입력하십시오.
1. **[!UICONTROL Enable messaging]** 필드를 `Yes`(으)로 설정하여 Adobe Commerce에서 체크아웃 중에 [!UICONTROL Fastlane] 브랜딩을 사용할지 여부를 지정합니다.
1. 변경 내용을 저장하려면 **[!UICONTROL Save Config]**&#x200B;을(를) 클릭합니다.
1. **[!UICONTROL System]** > **[!UICONTROL Cache Management]**(으)로 이동한 다음 **[!UICONTROL Flush Cache]**&#x200B;을(를) 클릭하여 모든 잘못된 캐시를 새로 고칩니다.
...
[!DNL Apple Pay]을(를) 사용하면 가맹점은 가맹점 계정당 최대 99개의 도메인을 지원하여 안전하고 빠르고 원활한 체크아웃 경험을 제공할 수 있습니다. **Safari**(macOS 및 iOS)에서 [!DNL Apple Pay] 단추는 체크아웃(빠른 체크아웃) 시작 시 및 최종 체크아웃 페이지에서 고객의 장치에서 결제, 연락처 및 배송 정보를 자동으로 채웁니다. **Chrome, Firefox 또는 Microsoft Edge**&#x200B;에서 [!DNL Apple Pay]은(는) **빠른 체크아웃**&#x200B;과(와) **최종 체크아웃 단계** 동안 데스크톱에서 QR 코드와 **iPhone**(iOS 18 이상)을(를) 통해 구매자가 Apple 결제 시트에서 결제를 완료할 수 있습니다. 빠른 체크아웃을 원하는 위치에 **[!UICONTROL Show Apple Pay on product detail page]** 또는 다른 배치가 활성화되어 있는지 확인하십시오.

>[!IMPORTANT]
>
> Apple은 확인되지 않은 도메인의 결제를 거부합니다. 가맹점은 Apple 결제 버튼이 사용되는 모든 도메인을 확인해야 합니다. 국가 제한이 적용됩니다. 도메인 확인 요구 사항에 대한 자세한 내용은 [Apple 결제 설명서](https://developer.apple.com/documentation/apple_pay_on_the_web)를 참조하세요.
...
1. 왼쪽 패널에서 **[!UICONTROL Sales]**&#x200B;을(를) 확장하고 **[!UICONTROL Payment Methods]**&#x200B;을(를) 선택합니다.
1. _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_섹션을 확장합니다.
1. _[!UICONTROL [!DNL Payment Services]]_섹션에서_[!UICONTROL Apple Pay]_ 섹션을 확장합니다.
1. **[!UICONTROL Title]**&#x200B;의 경우 필요한 경우 체크아웃 중에 표시된 대로 결제 방법 이름을 변경하려면 텍스트를 입력하십시오.
...
| [!UICONTROL Sort order] | 스토어 뷰 | 체크아웃 페이지에서 지정된 결제 방법에 대한 정렬 순서. `Numeric Only` 값 |
| [!UICONTROL Show Apple Pay on product detail page] | 스토어 뷰 | 제품 세부 정보 페이지에서 [!DNL Apple Pay]을(를) 활성화하거나 비활성화합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay in mini cart preview] | 스토어 뷰 | 미니 장바구니 미리 보기에서 [!DNL Apple Pay]을(를) 사용하거나 사용하지 않도록 설정합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Apple Pay on cart page] | 스토어 뷰 | 장바구니 페이지에서 [!DNL Apple Pay]을(를) 활성화하거나 비활성화합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Debug Mode] | 웹 사이트 | 디버그 모드를 활성화하거나 비활성화합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. 왼쪽 패널에서 **[!UICONTROL Sales]**&#x200B;을(를) 확장하고 **[!UICONTROL Payment Methods]**&#x200B;을(를) 선택합니다.
1. _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_섹션을 확장합니다.
1. _[!UICONTROL [!DNL Payment Services]]_섹션에서_[!UICONTROL Google Pay]_ 섹션을 확장합니다.
...
1. 쇼핑객이 Google 페이 시트 뒤에 별도의 **Google 페이 리뷰** 페이지를 볼 수 있는지 여부를 선택하려면 **[!UICONTROL Skip Review]**&#x200B;을(를) `Yes` 또는 `No`(으)로 설정하십시오. `Yes`(으)로 설정된 경우 지원되는 빠른 흐름은 Google Pay Sheet **에**&#x200B;배송 방법(클라이언트측 배송 콜백)을 표시하며 추가 검토 단계 없이 완료될 수 있습니다. `No`(으)로 설정하면 구매자는 결제하기 전에 검토 페이지에서 배송 및 합계를 확인할 수 있습니다.
1. **[!UICONTROL 3D Secure authentication]**(기본적으로 `Off`)을(를) 사용하려면 `Always` 또는 `When required`을(를) 선택하십시오.
1. 디버그 모드를 사용하려면 **[!UICONTROL Debug Mode]**&#x200B;에 대해 `Yes`을(를) 선택합니다(`No`이(가) 비활성화함).
...
1. 높이를 설정하려면 **[!UICONTROL Button Style]**&#x200B;에 정의된 높이의 기본값을 사용하십시오.
1. 변경 내용을 저장하려면 **[!UICONTROL Save Config]** 을(를) 클릭합니다.
1. **[!UICONTROL System]** > **[!UICONTROL Cache Management]**(으)로 이동한 다음 **[!UICONTROL Flush Cache]**&#x200B;을(를) 클릭하여 모든 잘못된 캐시를 새로 고칩니다.
...
| [!UICONTROL Sort order] | 스토어 뷰 | 체크아웃 페이지에서 지정된 결제 방법에 대한 정렬 순서. `Numeric Only` 값 |
| [!UICONTROL Show Google Pay on product detail page] | 스토어 뷰 | 제품 세부 정보 페이지에서 [!DNL Google Pay]을(를) 활성화하거나 비활성화합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay in mini cart preview] | 스토어 뷰 | 미니 장바구니 미리 보기에서 [!DNL Google Pay]을(를) 사용하거나 사용하지 않도록 설정합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Show Google Pay on cart page] | 스토어 뷰 | 장바구니 페이지에서 [!DNL Google Pay]을(를) 사용하거나 사용하지 않도록 설정합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Skip Review] | 스토어 뷰 | `[!UICONTROL Yes]`(으)로 설정된 경우 적격 [!DNL Google Pay] 빠른 플로우에서는 지급 시트 뒤에 별도의 검토 페이지를 생략할 수 있습니다. 운송 방법은 Google 지급 시트에 나타납니다. `[!UICONTROL No]`(으)로 설정하면 구매자가 검토 페이지로 이동하여 배송 및 합계를 확인합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
1. [나중에 결제 메시지](payments-options.md#pay-later-button)를 활성화/비활성화하려면 **[!UICONTROL Display Pay Later Message]**&#x200B;에 대해 `Yes`/`No`을(를) 선택하십시오.
   ...
   * [나중에 결제 메시지](payments-options.md#pay-later-button)를 사용하도록 설정하면 **[!UICONTROL PayPal Pay Later messaging]**&#x200B;의 스타일을 설정할 수 있도록 **[!UICONTROL Configure Messaging]** 모달 단추가 표시됩니다.
...
| [!UICONTROL Show buttons at start of checkout] | 스토어 뷰 | 체크아웃 흐름이 시작될 때 [!DNL PayPal payment buttons]을(를) 사용하거나 사용하지 않도록 설정하십시오. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Sort order] | 스토어 뷰 | 체크아웃 페이지에서 지정된 결제 방법에 대한 정렬 순서. `Numeric Only` 값 |
...
| [!UICONTROL Show buttons in mini cart preview] | 스토어 뷰 | 미니 장바구니 미리 보기에서 [!DNL PayPal payment buttons]을(를) 사용하거나 사용하지 않도록 설정합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
| [!UICONTROL App Switch] | 스토어 뷰 | 이 옵션은 **미국** 판매자와 구매자에게만 적용됩니다. 활성화하면 모든 Express PayPal 결제가 가능한 경우 PayPal 모바일 앱을 통해 진행됩니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Contact Preference] | 스토어 뷰 | 이 옵션은 **미국** 판매자와 구매자에게만 적용됩니다. **예**(으)로 설정하면 구매자의 전자 메일 및 전화 번호가 PayPal 결제 모달에 표시됩니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
로컬 결제 방법(LPM)은 기존 카드 기반 옵션과 함께 은행 송금 및 현지화된 결제 솔루션과 같은 지역별 및 로컬 결제 방법을 지원합니다. 판매자는 Commerce 구성 내에서 직접 사용 가능한 LPM을 활성화하거나 비활성화할 수 있습니다. 사용 가능한 방법에 대한 자세한 내용은 [결제 옵션](payments-options.md#local-payment-methods)을 참조하세요.

1. _관리자_ 사이드바에서 **[!UICONTROL Stores]** > _[!UICONTROL Settings]_>**[!UICONTROL Configuration]**(으)로 이동합니다.
1. 왼쪽 패널에서 **[!UICONTROL Sales]**&#x200B;을(를) 확장하고 **[!UICONTROL Payment Methods]**&#x200B;을(를) 선택합니다.
1. _[!UICONTROL FEATURED ADOBE PAYMENT SOLUTION]_섹션을 확장합니다.
1. _[!UICONTROL Payment Services]_섹션에서_[!UICONTROL Local Payment Methods]_ 섹션을 확장합니다.
1. **[!UICONTROL Active]**&#x200B;에 대해 LPM을 활성화하려면 `Yes`을(를) 선택하고 비활성화하려면 `No`을(를) 선택하십시오.
1. **[!UICONTROL Title]**&#x200B;의 경우 체크아웃 중에 결제 방법 이름으로 표시할 텍스트를 입력하십시오. 이 제목은 판매 주문 그리드에도 나타납니다.
1. **[!UICONTROL Allowed Payment Methods]**&#x200B;의 경우 제공할 결제 방법을 선택하십시오. 사용 가능한 방법은 구매자의 청구 주소와 웹 사이트의 기본 통화에 따라 다릅니다.
1. **[!UICONTROL Sort Order]**&#x200B;의 경우 사용 가능한 결제 방법 목록 내에서 LPM의 표시 위치를 결정하는 숫자 값을 입력하십시오.
1. 변경 내용을 저장하려면 **[!UICONTROL Save Config]**&#x200B;을(를) 클릭합니다.
1. **[!UICONTROL System]** > **[!UICONTROL Cache Management]**(으)로 이동한 다음 **[!UICONTROL Flush Cache]**&#x200B;을(를) 클릭하여 모든 잘못된 캐시를 새로 고칩니다.
...
| 필드 | 범위 | 설명 |
|---|---|---|
| [!UICONTROL Active] | 웹 사이트 | 로컬 결제 방법을 활성화하거나 비활성화합니다. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
| [!UICONTROL Title] | 스토어 뷰 | 체크아웃 시 및 판매 주문 그리드에 표시되는 결제 방법 이름. 옵션: [!UICONTROL text field] |
| [!UICONTROL Allowed Payment Methods] | 웹 사이트 | 제공할 LPM을 선택합니다. 메서드는 청구 주소 및 웹 사이트 통화가 메서드의 요구 사항과 일치하는 고객에게만 표시됩니다. 옵션: `Bancontact` / `BLIK` / `eps` / `iDEAL` / `MyBank` / `Przelewy24` |
| [!UICONTROL Sort Order] | 스토어 뷰 | 사용 가능한 결제 방법 목록 내의 LPM 그룹 표시 위치. 이 설정은 그룹으로서 모든 LPM에 적용됩니다. 개별 LPM은 독립적으로 정렬할 수 없습니다. `Numeric Only` 값 |

>[!NOTE]
>
>각 LPM(Local Payment Method)에는 특정 국가 및 통화 요구 사항이 있습니다. 결제 방법은 고객의 청구 주소 국가와 웹 사이트의 기본 통화가 해당 요구 사항과 일치하는 경우에만 나타납니다. 예를 들어 기본 통화가 EUR인 경우 벨기에 청구 주소가 있는 고객에 대해서만 Bancontact가 나타납니다.

## 라인 항목

라인 품목은 제품 세부 사항, 수량 및 가격을 포함한 자세한 주문 정보를 PayPal로 보냅니다. 이 기능은 기본적으로 활성화되어 있습니다. 자세한 내용은 [라인 항목](line-items.md)을 참조하세요.

### 구성 옵션

| 필드 | 범위 | 설명 |
|---|---|---|
| [!UICONTROL Line Items Enabled] | 웹 사이트 | PayPal로 라인 항목 및 금액 분류 전송을 활성화하거나 비활성화합니다. [!DNL Payment Services]에서 지원하지 않는 사용자 지정 요금을 추가하는 타사 확장을 사용하는 경우 이 설정을 사용하지 않도록 설정하십시오. 옵션: `[!UICONTROL Yes]` / `[!UICONTROL No]` |
...
역할을 관리하는 방법은 [사용자 역할](https://experienceleague.adobe.com/en/docs/commerce-admin/systems/user-accounts/permissions-user-roles)을 참조하세요.
...
웹 사이트, 스토어 및 스토어 보기의 계층 구조에 대한 자세한 내용은 [사이트, 스토어 및 보기 범위](https://experienceleague.adobe.com/en/docs/commerce-admin/start/setup/websites-stores-views)를 참조하십시오.
