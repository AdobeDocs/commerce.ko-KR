...
* **고급** - 현재 [완전히 지원되는 국가](compatibility.md#standard-vs-advanced-payment-services-experience)에서 사용 가능한 모든 [결제 옵션](../payment-services/payments-options.md)을 사용할 수 있습니다. 실시간 결제를 활성화하려면 온보딩 중에 [고급 온보딩 옵션](../payment-services/production.md#advanced-onboarding)을 선택하세요.
...
[!DNL Apple Pay]을(를) 사용하면 가맹점은 안전하고 간소화된 체크아웃 환경(가맹점 계정당 최대 99개의 도메인)을 제공하여 전환을 늘릴 수 있습니다.

* **Safari(macOS 및 iOS)** — [!DNL Apple Pay] 버튼은 체크아웃(빠른 체크아웃) 시작 및 최종 체크아웃 페이지에서 고객의 Apple 디바이스에서 직접 저장된 결제, 연락처 및 배송 세부 정보를 자동으로 채웁니다.
* **Chrome, Firefox 및 Microsoft Edge** — 구매자는 **빠른 체크아웃** 및 **최종 체크아웃 단계** 동안 모두 [!DNL Apple Pay]을(를) 사용할 수 있습니다. 데스크톱에서 **QR 코드**&#x200B;가 표시되므로 쇼핑객이 지갑 흐름을 열기 위해 카메라 앱을 사용하여 **iPhone**(iOS 18 이상)의 Apple Pay 시트에서 결제를 완료합니다.

이 흐름에 대한 Apple 개요는 [전자 지갑의 새로운 기능 및 [!DNL Apple Pay]](https://developer.apple.com/videos/play/wwdc2024/10108/?time=35){target=_blank}(Apple 개발자, WWDC24)을 참조하십시오.
...
고객은 [!DNL Apple Pay] 빠른 체크아웃 중에 **단일 장바구니 가격 규칙(쿠폰) 코드**&#x200B;을(를) 적용하거나 제거할 수 있습니다.

>[!NOTE]
>
> Apple Pay 도메인 인증 인증서가 이미 [!DNL Payment Services] 코드에 포함되어 있습니다. `/.well-known/apple-developer-merchantid-domain-association` 경로가 200 응답 코드를 반환하는지 확인하십시오. **Apple Pay 도메인 확인** 인증서에 대한 자세한 내용은 [Apple Pay와 통합에 대한 PayPal 개발자 설명서](https://developer.paypal.com/docs/checkout/apm/apple-pay/#download-and-host-sandbox-domain-association-file)를 참조하십시오.

자세한 내용은 [설정](configure-admin.md#apple-pay)을 참조하세요.

#### [!DNL Apple Pay] Express 제한 사항

[!DNL Apple Pay] 페이시트의 **프로모션 코드**

* [!DNL Apple Pay] 지불표에 입력한 프로모션 코드는 빠른 흐름에만 적용됩니다. 표준 체크아웃 페이지에서 [!DNL Apple Pay]을(를) 선택한 경우에는 적용되지 않습니다.
* [!DNL Apple Pay] 페이시트당 **one** 프로모션 코드만 적용할 수 있습니다.
* [!DNL Apple Pay] 검토 페이지가 없습니다. 구매자가 급부표에서 직접 구매를 완료합니다.
* 쇼핑객이 [!DNL Apple Pay] 페이시트를 닫았다가 다시 열면 이전에 입력한 프로모션 코드가 기억되지 않습니다. 할인 금액만 합계에 반영됩니다.

**비Safari 브라우저**

* [!DNL Apple Pay] 단추가 빠른 체크아웃 흐름 또는 표준 체크아웃 흐름의 Android 장치에서 렌더링되지 않습니다.
* **가상 제품**&#x200B;의 경우 [!DNL Apple Pay] 결제 시트에 배송 주소를 묻는 메시지가 표시됩니다. 쇼핑객이 결제를 승인할 때까지 Apple에서 청구 주소를 제공하지 않기 때문에 이 주소는 합계를 계산하기 위한 청구 주소의 최적 예상 비용으로 사용됩니다.
...
[!DNL Google Pay] **express** 체크아웃은 Google 결제 시트에 **배송 방법을 표시하고** 선택적 **검토** 단계를 지원하고(**[검토 건너뛰기](configure-admin.md#google-pay)** 구성) 체크아웃 중에 **홍보 코드** 필드를 포함할 수 있습니다.
...
#### [!DNL Google Pay] Express 제한 사항

**급여표에서 배송**

* **시트 내 배달** 동작(클라이언트측 배달 콜백)은 [Google 결제 구성](configure-admin.md#google-pay)에서 **[!UICONTROL Skip Review]**&#x200B;이(가) `Yes`(으)로 설정된 경우에만 사용할 수 있습니다.

[!DNL Google Pay] 페이시트의 **프로모션 코드**

* [!DNL Google Pay] 지불표에 입력한 프로모션 코드는 빠른 흐름에만 적용됩니다. 표준 체크아웃 페이지에서 [!DNL Google Pay]을(를) 선택한 경우에는 적용되지 않습니다.
* 스토어에서 주문당 여러 개의 쿠폰을 허용하는 경우에도 [!DNL Google Pay] 페이시트당 **one** 프로모션 코드만 적용할 수 있습니다. (여러 쿠폰은 표준 장바구니 및 체크아웃에서 계속 지원됩니다.)
* 프로모션 코드는 기프트 카드 상품에 적용 할 수 없습니다.
* 프로모션 코드 필드가 **Android 장치에서 지원되지 않음**.
...
* 자동 [장바구니 가격 규칙](https://experienceleague.adobe.com/ko/docs/commerce-admin/marketing/promotions/cart-rules/price-rules-cart)이 적용되는 경우(예: &quot;$50 off when spending us $200&quot;), 이 규칙은 쇼핑객이 급여시트에서 적용하는 모든 코드와 결합됩니다. [!DNL Google Pay] 지불 시트에 표시되는 합계는 주문 요약과 다를 수 있습니다.
...
저장소 구성 또는 [!DNL Payment Services] 홈에서 [!UICONTROL PayPal payment buttons]을(를) 구성할 수 있습니다.
...
[설정](configure-admin.md#paypal-payment-buttons) 구성을 업데이트하여 [!DNL Pay Later] 메시지를 사용하지 않도록 설정하거나 사용하도록 설정하는 방법에 대해 알아봅니다.

##### 선택 사항입니다. 나중에 결제 메시지 구성

[나중에 결제](configure-admin.md#paypal-payment-buttons)에 대한 **메시지 구성**&#x200B;을 통해 판매자는 이 결제 옵션의 기본 스타일을 수정할 수 있습니다. [설정](configure-admin.md#paypal-payment-buttons) 구성에서 **[!UICONTROL Display Pay Later Message]**&#x200B;을(를) `Yes`(으)로 설정하면 **[!UICONTROL PayPal Pay Later messaging]**&#x200B;의 스타일을 설정할 수 있도록 **[!UICONTROL Configure Messaging]** 모달 단추가 표시됩니다.

![나중에 결제 메시지](assets/pay-later-messaging.png){width="500" zoomable="yes"}

### PayPal 결제 버튼에 대한 서버측 배송 콜백

PayPal, Pay Later 및 Venmo 결제 방법에서는 PayPal이 Commerce 인스턴스와 직접 통신하여 배송 옵션을 검색하고 합계를 실시간으로 계산할 수 있는 [서버측 배송 콜백](https://developer.paypal.com/docs/multiparty/checkout/standard/customize/shipping-module/)을 사용합니다.

이 서버측 접근 방식을 사용하면 [!DNL Payment Services]에서 주문 확인 팝업을 건너뛸 수 있으므로 더 빠르고 간소화된 구매 환경을 제공할 수 있습니다. 배송비 및 세금은 콜백을 통해 동적으로 계산되므로 구매자는 PayPal 또는 Venmo 검토 페이지에서 직접 정확한 합계를 볼 수 있습니다.

>[!NOTE]
>
>콜백 끝점은 공개적으로 사용할 수 있어야 하며 5초 이내에 응답해야 합니다. 응답 시간이 이 제한을 초과하면 PayPal이 팝업에 오류 메시지를 표시합니다. 이러한 지불 방법을 로컬에서 테스트하는 방법에 대한 자세한 내용은 [로컬 개발 환경에서 테스트](test-validate.md#test-on-local-development-environments)를 참조하십시오.

### PayPal 결제 버튼만 사용

스토어를 프로덕션 모드로 빠르게 전환하려면 PayPal 신용카드 결제 옵션을 사용하는 대신 _only_ PayPal 결제 버튼(Venmo, PayPal 등)을 구성할 수 있습니다.
...
1. _[!UICONTROL Credit card fields]_&#x200B;섹션에서&#x200B;**[[!UICONTROL Show on checkout page]](configure-admin.md#credit-card-fields)**&#x200B;옵션을_&#x200B;해제&#x200B;_하고 [기존 신용 카드 공급자 계정](https://experienceleague.adobe.com/ko/docs/commerce-admin/stores-sales/payments/payments#payments)을 사용하세요.

## 로컬 결제 방법

로컬 결제 방법(LPM)은 기존 카드 기반 옵션과 함께 은행 송금 및 현지화된 결제 솔루션과 같은 지역별 및 로컬 결제 방법을 지원합니다. 판매자는 Commerce 구성 내에서 직접 사용 가능한 LPM을 활성화하거나 비활성화할 수 있습니다. LPM은 Adobe의 결제 기능을 확장하고, 유럽 시장의 요구를 지원하며, 체크아웃 현지화를 개선하고, 전환, 판매자 채택 및 구매자 만족도를 높이는 데 도움을 줍니다.

사용 가능한 LPM은 다음과 같습니다.
...
LPM은 청구 주소 및 웹 사이트의 기본 통화에 따라 고객에게 표시됩니다. 결제 방법은 두 조건이 모두 결제 방법의 요구 사항과 일치하는 경우에만 나타납니다.

자세한 내용은 [로컬 결제 방법 구성](configure-admin.md#local-payment-methods)을 참조하십시오.

## 빠른 체크아웃 단추

더 빠른 체크아웃 경험을 장려하기 위해 체크아웃 플로우 시작 시 빠른 결제 옵션을 사용할 수 있습니다. 고객은 PayPal, PayPal Pay Later, Venmo, Apple Pay 또는 Google Pay를 사용하여 구매를 완료할 수 있습니다.

활성화되면 체크아웃 프로세스 초기에 빠른 체크아웃 버튼이 표시돼 디지털 지갑 결제수단을 선호하는 고객에게 더 빠른 구매경로를 제공한다.

빠른 체크아웃 버튼을 사용하려면 각 결제 방법을 개별적으로 구성하십시오.

* **PayPal 및 나중에 결제**: [PayPal 결제 단추](configure-admin.md#paypal-payment-buttons) 설정에서 **[!UICONTROL Show buttons at start of checkout]**&#x200B;을(를) 사용하도록 설정합니다.

* **Apple 결제**: [Apple 결제](configure-admin.md#apple-pay) 설정에서 **[!UICONTROL Show Apple Pay at start of checkout]**&#x200B;을(를) 활성화합니다.

* **Google 결제**: [Google 결제](configure-admin.md#google-pay) 설정에서 **[!UICONTROL Show Google Pay at start of checkout]**&#x200B;을(를) 활성화합니다.

>[!NOTE]
>
>결제 방법 사용 가능 여부는 구매자의 위치에 따라 다릅니다. 샌드박스 테스트의 경우 [구매자의 국가](sandbox.md#buyers-country) 구성을 사용하여 다른 지역을 시뮬레이션하십시오. 예를 들어 벤모는 미국에서만 사용할 수 있습니다. 후불 결제는 미국과 영국에서 이용 가능합니다.
